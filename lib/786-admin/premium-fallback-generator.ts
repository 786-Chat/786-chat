...
export default function Page(){
 ...
 return <main className={"site layout-${layout} industry-${industry}"}>
  <header className="topbar"><a className="brand" href="#top">{projectTitle}</a><button className="menuButton" onClick={()=>setOpen(!open)}>Menu</button><nav className={open?"open":""}>{sections.map((item,index)=><a key={item} href={'#'+id(item)} onClick={()=>{setActive(index);setOpen(false)}}>{item}</a>)}</nav></header>
  <section className="hero" id="top"><div className="heroCopy"><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.headline}</h1><p className="lead">{copy.description}</p><a className="cta" href={'#'+id(sections[0])}>{copy.cta}</a></div><div className="heroVisual"><span className="visualWord">${industry === "food" ? "HOT" : industry === "industrial" ? "BUILD" : "786"}</span><div className="orb one"/><div className="orb two"/></div></section>
  <section className="sectionIndex">{sections.map((item,index)=><button key={item} className={active===index?"active":""} onClick={()=>{setActive(index);document.getElementById(id(item))?.scrollIntoView({behavior:"smooth"})}}><span>{String(index+1).padStart(2,"0")}</span>{item}</button>)}</section>
  <section className="contentGrid">{sections.map((item,index)=><article id={id(item)} key={item} className={active===index?"activeCard":""}><span className="number">{String(index+1).padStart(2,"0")}</span><h2>{item}</h2><p>{${industry === "food" ? "['Stone-baked favourites made with fresh ingredients.','Simple choices, bold flavour and generous portions.','Eat in, collect or order exactly how you like.','A welcoming local shop made for repeat visits.]" : "['Designed around the real needs of this project.','Clear information with purposeful interaction.','Responsive structure for every screen size.','A distinct section with its own visual rhythm.]"}[index] || 'Created specifically for '+projectTitle}</p></article>)}</section>
...
function globalCss(p: Palette, layout: Layout, rotation: number, accentPosition: number) {
  return `@tailwind base;@tailwind components;@tailwind utilities;:root{--bg:${p.bg};--surface:${p.surface};--ink:${p.ink};--muted:${p.muted};--primary:${p.primary};--secondary:${p.secondary};--radius:${p.radius};--rotate:${rotation}deg}*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--ink);font-family:${p.font}}button,a{font:inherit}.site{min-height:100vh;overflow:hidden;background:radial-gradient(circle at ${["15% 10%","85% 12%","20% 85%","90% 80%"][accentPosition]},color-mix(in srgb,var(--secondary) 24%,transparent),transparent 32%),var(--bg)}a{color:inherit;text-decoration:none}.topbar{width:min(1240px,calc(100% - 32px));margin:auto;padding:24px 0;display:flex;align-items:center;justify-content:space-between;gap:28px;border-bottom:1px solid color-mix(in srgb,var(--ink) 16%,transparent)}.brand{font-size:20px;font-weight:900}.topbar nav{display:flex;gap:22px;flex-wrap:wrap}.topbar nav a{font-size:13px;font-weight:800;color:var(--muted)}.menuButton{display:none}.hero{width:min(1240px,calc(100% - 32px));margin:auto;min-height:68vh;display:grid;grid-template-columns:1.05fr .95fr;align-items:center;gap:7vw}.eyebrow{font-size:12px;font-weight:950;letter-spacing:.22em;text-transform:uppercase;color:var(--primary)}h1{font-size:clamp(54px,8vw,122px);line-height:.88;letter-spacing:-.065em;margin:20px 0 28px;max-width:920px}.lead{max-width:680px;font-size:clamp(18px,2.1vw,28px);line-height:1.45;color:var(--muted)}.cta{display:inline-flex;margin-top:28px;padding:15px 22px;background:var(--primary);color:white;border-radius:999px;font-weight:900}.heroVisual{position:relative;min-height:430px;border-radius:var(--radius);background:linear-gradient(145deg,var(--primary),var(--secondary));transform:rotate(var(--rotate));box-shadow:30px 36px 90px rgba(0,0,0,.24);overflow:hidden}.visualWord{position:absolute;inset:auto 24px 16px;font-size:clamp(70px,12vw,180px);font-weight:950;letter-spacing:-.09em;color:rgba(255,255,255,.2)}.orb{position:absolute;border-radius:50%;background:rgba(255,255,255,.28);filter:blur(1px)}.orb.one{width:180px;height:180px;right:-35px;top:-25px}.orb.two{width:90px;height:90px;left:15%;bottom:12%}.sectionIndex{width:min(1240px,calc(100% - 32px));margin:0 auto 34px;display:flex;gap:10px;overflow:auto;padding-bottom:8px}.sectionIndex button{white-space:nowrap;border:1px solid color-mix(in srgb,var(--ink) 15%,transparent);background:transparent;color:var(--muted);padding:12px 16px;border-radius:999px;font-weight:800}.sectionIndex button span{margin-right:9px}.sectionIndex button.active{background:var(--ink);color:var(--bg)}.contentGrid{width:min(1240px,calc(100% - 32px));margin:auto;display:grid;grid-template-columns:repeat(12,1fr);gap:18px;padding:30px 0 100px}.contentGrid article{grid-column:span 6;min-height:260px;padding:30px;border:1px solid color-mix(in srgb,var(--ink) 14%,transparent);border-radius:var(--radius);background:color-mix(in srgb,var(--surface) 88%,transparent);transition:.25s}.contentGrid article:nth-child(3n+1){grid-column:span 7}.contentGrid article:nth-child(3n+2){grid-column:span 5}.contentGrid article:hover,.contentGrid article.activeCard{transform:translateY(-6px);border-color:var(--primary)}.number{font-weight:950;color:var(--primary)}h2{font-size:clamp(30px,4vw,54px);letter-spacing:-.04em;margin:55px 0 12px}.contentGrid p{color:var(--muted);font-size:17px;line-height:1.6}footer{width:min(1240px,calc(100% - 32px));margin:auto;padding:28px 0 45px;border-top:1px solid color-mix(in srgb,var(--ink) 16%,transparent);display:flex;justify-content:space-between;color:var(--muted)}.layout-poster .hero{grid-template-columns:1fr}.layout-poster .heroVisual{position:absolute;width:38vw;right:-8vw;top:15vh;z-index:0;opacity:.76}.layout-poster .heroCopy{position:relative;z-index:1}.layout-poster h1{font-size:clamp(70px,12vw,180px)}.layout-editorial .hero{grid-template-columns:1.2fr .55fr}.layout-editorial h1{font-family:Georgia,serif;font-weight:500}.layout-mosaic .heroVisual{border-radius:45% 12% 38% 16%}.layout-storefront .heroVisual{border-radius:50% 50% 8% 8%}.layout-catalogue .contentGrid article{grid-column:span 3;min-height:330px}.layout-story .contentGrid{display:block}.layout-story .contentGrid article{margin:0 0 24px;display:grid;grid-template-columns:120px 1fr;align-items:center}.layout-command{display:grid;grid-template-columns:250px 1fr}.layout-command .topbar{position:sticky;top:0;width:auto;height:100vh;margin:0;padding:28px;display:flex;flex-direction:column;align-items:flex-start;justify-content:flex-start;background:var(--surface);z-index:5}.layout-command .topbar nav{display:flex;flex-direction:column}.layout-command .hero,.layout-command .sectionIndex,.layout-command .contentGrid,.layout-command footer{grid-column:2}.layout-command .hero{min-height:72vh}.layout-command .sectionIndex{display:none}@media(max-width:800px){.topbar{align-items:flex-start}.menuButton{display:block;border:0;background:var(--ink);color:var(--bg);padding:9px 13px;border-radius:999px}.topbar nav{display:none;position:absolute;z-index:20;top:72px;left:16px;right:16px;padding:18px;background:var(--surface);border-radius:var(--radius);box-shadow:0 20px 50px rgba(0,0,0,.2)}.topbar nav.open{display:grid}.hero{grid-template-columns:1fr;min-height:auto;padding:70px 0}.heroVisual{min-height:260px}.contentGrid article,.contentGrid article:nth-child(n){grid-column:1/-1}.layout-poster .heroVisual{position:relative;width:100%;right:auto;top:auto}.layout-catalogue .contentGrid article{grid-column:1/-1}.layout-command{display:block}.layout-command .topbar{position:relative;width:min(100% - 32px,1240px);height:auto;margin:auto;flex-direction:row;justify-content:space-between;background:transparent}.layout-command .hero,.layout-command .contentGrid,.layout-command footer{display:grid}.layout-story .contentGrid article{display:block}footer{gap:20px;flex-direction:column}}`
}
