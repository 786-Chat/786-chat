...
type Industry = "food" | "industrial" | "education" | "medical" | "travel" | "aviation" | "retail" | "technology" | "general"
...
function chooseDesign(seed: string, industry: Industry, prompt: string) {
  ...
  if (industry === "travel") layout = (["editorial", "story", "mosaic", "split"] as Layout[])[base % 4]
  const text = positivePrompt(prompt).toLowerCase()
  const palette = /pearl|ocean blue|burnt orange/.test(text) ? PALETTES[2] : PALETTES[base % PALETTES.length]
...
function pageSource(title: string, industry: Industry, features: string[], routes: string[], route: string, layout: Layout, copy: ReturnType<typeof copyFor>) {
  const visualWord = industry === "food" ? "TASTE" : industry === "industrial" ? "BUILD" : industry === "travel" ? "GO" : industry === "aviation" ? "FLY" : "786"
  ...
  const cards = routeFeatures.map((item, index) => `<article><span className="number">${String(index + 1).padStart(2, "0")}</span><h2>${escapeJsxText(item)}</h2><p>${escapeJsxText(route === "/" ? `Explore ${item.toLowerCase()} through ${title}.` : `${routeName} content created specifically for ${title}.`)}</p></article>`).join("\n    ")
  const headline = route === "/" ? copy.headline : `${routeName} at ${title}`
  const description = route === "/" ? copy.description : `A complete ${routeName.toLowerCase()} page inside the ${title} customer application.`
 ...
 return (
  <main className="site layout-${layout} industry-${industry}">
   ...
   <footer><strong>${escapeJsxText(title)}</strong><span>${escapeJsxText(routeName)} · Customer application</span></footer>
  </main>
 )
}`
}
