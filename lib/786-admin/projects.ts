...
export async function listProjects(ownerEmail: string): Promise<AdminProjectListItem[]> {
...
export async function getProject(id: string, ownerEmail: string): Promise<AdminProject | null> {
  const rows = (await sql`
    SELECT id, owner_email, title, description, prompt,
...
export async function createProject(
  ...
  const rows = (await sql`
    INSERT INTO admin_projects
      (owner_email, title, description, prompt, preview_state, metadata)
    VALUES (
    ...
    RETURNING id, owner_email, title, description, prompt,
              preview_state, metadata, created_at, updated_at
  `) as unknown as AdminProject[]
...
export async function updateProject(
  ...
  const previewPatch =
    patch.preview_state_patch !== undefined ? JSON.stringify(patch.preview_state_patch) : null
  const metadataPatch =
    patch.metadata_patch !== undefined ? JSON.stringify(patch.metadata_patch) : null
...
export async function deleteProject(id: string, ownerEmail: string): Promise<boolean> {
  const rows = (await sql`
...
export async function getFiles(projectId: string): Promise<AdminProjectFileMap> {
  const rows = (await sql`
    SELECT path, content
  ...
  const files: AdminProjectFileMap = {}
...
export async function upsertFiles(projectId: string, files: AdminProjectFileMap): Promise<number> {
  let count = 0
...
export async function getProjectWithData(
  ...
  const [files, messages] = await Promise.all([getFiles(project.id), getMessages(project.id)])
...
export async function persistProjectAtomic(
  ...
  if (!isCreate) {
    ...
    if (!owned) throw new Error("Project not found or not owned by this admin")
  } else if (!input.title?.trim()) {
    throw new Error("title is required to create a project")
  }

  const previewPatch =
    input.preview_state_patch !== undefined ? JSON.stringify(input.preview_state_patch) : null
  const metadataPatch =
    input.metadata_patch !== undefined ? JSON.stringify(input.metadata_patch) : null
  ...
  if (isCreate) {
    queries.push(sql`
      INSERT INTO admin_projects
        (id, owner_email, title, description, prompt, preview_state, metadata)
      VALUES (
        ${projectId},
        ${owner},
        ${input.title!},
  ...
  queries.push(sql`UPDATE admin_projects SET updated_at = NOW() WHERE id = ${projectId}`)

  await transaction(queries)

  const result = await getProjectWithData(projectId, owner)
  if (!result) throw new Error("Persistence committed but project could not be read back")
  return result
}
