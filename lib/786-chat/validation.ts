...
function stripClientMetadataExport(content: string) {
  ...
  for (; cursor < content.length && depth > 0; cursor += 1) {
    ...
    if (blockComment) { if (character === "*" && next === "/") { blockComment = false; cursor += 1 }; continue }
    if (quote) {
    ...
    if (character === "/" && next === "/") { lineComment = true; cursor += 1; continue }
    if (character === "/" && next === "*") { blockComment = true; cursor += 1; continue }
  ...
  return `${content.slice(0, declaration.index)}${content.slice(end)}`
    .replace(/^\s*import\s+type\s+\{\s*Metadata\s*\}\s+from\s+["']next["'];?\s*$/m, "")
    .replace(/\n{3,}/g, "\n\n")
}
...
export function normalizeGeneratedMetadataBoundaries(files: Record<string, string>) {
  return Object.fromEntries(Object.entries(files).map(([path, content]) => [
    path,
    /^(?:src\/)?app\/(?:.*\/)?(?:layout|page)\.(?:tsx|jsx)$/.test(path) ? stripClientMetadataExport(content) : content,
  ]))
}

export function normalizeGeneratedImports(files: Record<string, string>) {
  return Object.fromEntries(Object.entries(files).map(([path, content]) => {
    if (!/\.(?:tsx?|jsx?)$/.test(path) || !/from\s+["']lucide-react["']/.test(content)) return [path, content]
    const normalized = content.replace(/import\s*\{([^}]+)\}\s*from\s*["']lucide-react["']/g, (statement, names: string) => {
      let normalizedNames = names
      for (const [unsupported, supported] of Object.entries(LUCIDE_COMPATIBILITY_ALIASES)) {
        normalizedNames = normalizedNames.replace(new RegExp(`\\b${unsupported}\\b(?!\\s+as\\b)`, "g"), `${supported} as ${unsupported}`)
      }
...
export function normalizeGeneratedClientBoundaries(files: Record<string, string>) {
  return Object.fromEntries(Object.entries(files).map(([path, content]) => {
    if (!/\.(?:tsx|jsx)$/.test(path) || /^app\/api\//.test(path) || !/\b(?:useState|useEffect|useReducer|useRef|useLayoutEffect|useCallback|useMemo)\s*\(/.test(content) || /^\s*["']use client["'];?/m.test(content)) return [path, content]
    return [path, `"use client"\n\n${content}`]
  }))
}

export function normalizeGeneratedAuthLinks(specification: ProjectSpecification, files: Record<string, string>) {
  ...
  return Object.fromEntries(Object.entries(files).map(([path, content]) => {
    ...
    const normalized = content
      .replace(/(\bhref\s*=\s*(?:\{\s*)?)(["'`])(\/(?:forgot-password|register|signup|sign-up))\2((?:\s*\})?)/gi, (_match, prefix: string, quote: string, route: string, suffix: string) => `${prefix}${quote}${replacements[route.toLowerCase()]}${quote}${suffix}`)
      .replace(/(\b(?:href|to)\s*:\s*)(["'`])(\/(?:forgot-password|register|signup|sign-up))\2/gi, (_match, prefix: string, quote: string, route: string) => `${prefix}${quote}${replacements[route.toLowerCase()]}${quote}`)
    return [path, normalized]
  }))
}
...
function routeFileCandidates(route: string) {
  ...
  return [`app/${suffix}.tsx`, `app/${suffix}.ts`, `app/${suffix}.jsx`, `app/${suffix}.js`, `src/app/${suffix}.tsx`, `src/app/${suffix}.ts`, `src/app/${suffix}.jsx`, `src/app/${suffix}.js`]
}

function source(files: Record<string, string>) {
  return Object.entries(files).filter(([path]) => /\.(?:tsx?|jsx?|css)$/.test(path)).map(([path, content]) => `/* ${path} */\n${content}`).join("\n")
}
  ...
  for (const [path, content] of Object.entries(files)) {
    ...
    for (const pattern of routePatterns) {
      for (const match of content.matchAll(pattern)) {
        ...
        const route = normalizeDynamicRoute(href.split(/[?#]/, 1)[0].replace(/\/+$/, "") || "/")
...
export function validateGeneratedProject(specification: ProjectSpecification, files: Record<string, string>): ProjectValidation {
  ...
  for (const [path, content] of Object.entries(files)) {
    if (/^(?:src\/)?app\/(?:.*\/)?(?:layout|page)\.(?:tsx|jsx)$/.test(path) && /^\s*["']use client["'];?/m.test(content) && /\bexport\s+const\s+metadata\b/.test(content)) {
      errors.push(`Client component cannot export Next.js metadata: ${path}`)
    }
  }

  const hasRootPage = routeFileCandidates("/").some((path) => typeof files[path] === "string" && files[path].trim())
  if (!hasRootPage) errors.push("Missing required Next.js root route: / (app/page.tsx or src/app/page.tsx)")

  for (const route of specification.routes) {
    if (route === "/" && !hasRootPage) continue
    if (!routeFileCandidates(route).some((path) => typeof files[path] === "string" && files[path].trim())) errors.push(`Missing requested route: ${route}`)
  }

  for (const route of internalHrefRoutes(files)) {
    if (!routeFileCandidates(route).some((path) => typeof files[path] === "string" && files[path].trim())) errors.push(`Internal navigation points to missing route: ${route}`)
  }
  ...
  if (specification.requiredInteractions.includes("submit-booking")) {
    const bookingSource = routeFileCandidates("/booking").map((path) => files[path] || "").join("\n")
    if (!/<form\b/i.test(bookingSource) || !/(?:onSubmit|action\s*=|formAction)/i.test(bookingSource) || !/(?:date|time|appointment|booking)/i.test(bookingSource)) errors.push("Booking route is missing a functional booking form with submit handling.")
  }
  ...
  if (specification.systemBlueprint) {
    const requiredSystemFiles = ["shared/contracts.ts", "lib/server/tenant.ts", "lib/server/validation.ts", "sql/schema.sql", ...specification.systemBlueprint.apiResources.map((resource) => `app/api/${resource}/route.ts`), ...specification.systemBlueprint.apiResources.map((resource) => `app/api/${resource}/[id]/route.ts`)]
    for (const path of requiredSystemFiles) if (typeof files[path] !== "string" || !files[path].trim()) errors.push(`Missing required system file: ${path}`)
    const schema = files["sql/schema.sql"] || ""
    const tenantGuard = files["lib/server/tenant.ts"] || ""
    if (specification.systemBlueprint.tenantScoped) {
      if (!/\bcompany_id\b/i.test(schema)) errors.push("Tenant-scoped schema is missing company_id.")
      if (!/company[_A-Z]?id|company_id/i.test(tenantGuard) || !/unauthor|forbidden|access|permission|role/i.test(tenantGuard)) errors.push("Server tenant guard does not enforce company ownership.")
    }
    if (!/CREATE\s+TABLE/i.test(schema) || !/audit/i.test(schema) || !/\bREFERENCES\b/i.test(schema) || !/\bCREATE\s+(?:UNIQUE\s+)?INDEX\b/i.test(schema) || !/\bTIMESTAMPTZ\b/i.test(schema)) errors.push("Operational PostgreSQL schema or audit storage is incomplete.")
    for (const resource of specification.systemBlueprint.apiResources) {
      ...
      if (!/export\s+async\s+function\s+GET/.test(collectionApi) || !/export\s+async\s+function\s+POST/.test(collectionApi) || !/export\s+async\s+function\s+GET/.test(itemApi) || !/export\s+async\s+function\s+PATCH/.test(itemApi) || !/export\s+async\s+function\s+DELETE/.test(itemApi)) errors.push(`System CRUD API is not implemented: ${resource}`)
    }
    const acceptance = assessGeneratedSystem({ entities: specification.systemBlueprint.entities, apiResources: specification.systemBlueprint.apiResources, workflows: specification.systemBlueprint.workflows, tenantScoped: specification.systemBlueprint.tenantScoped, platforms: specification.platforms }, files)
    errors.push(...acceptance.errors)
    const domainAcceptance = assessDomainAcceptance(specification.systemBlueprint.id, files)
