...
test("runtime acceptance is internal and the visible route returns customers to the AI workspace", async () => {
  const page = await read("app/786-admin/runtime-acceptance/page.tsx")
  const route = await read("app/api/786-chat/system-acceptance/runtime/route.ts")
  assert.match(page, /redirect\("\/786\.chat"\)/)
  assert.doesNotMatch(page, /Runtime acceptance matrix|Run acceptance|crm|manufacturing|pest-iot/i)
  assert.match(route, /RUN_PHASE_3_RUNTIME_ACCEPTANCE/)
  assert.match(route, /isAdminUser/)
})
