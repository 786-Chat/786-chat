    ...
    <main className="relative flex h-screen min-w-[1000px] overflow-hidden bg-[#050813] text-slate-100">
      ...
      <aside className={`relative z-20 flex shrink-0 flex-col border-r border-[#1b2940] bg-[#070c18] transition-[width] ${sidebarCollapsed ? "w-[70px]" : "w-[176px]"}`}>
        ...
        <nav className="flex-1 px-2 py-3">
            ...
            return (
              <div key={item.label}>
                {index === 1 && !sidebarCollapsed && (
                  <p className="mb-2 mt-5 px-2 text-[9px] font-bold uppercase tracking-[.16em] text-slate-600">Project</p>
                )}
                <button
                  type="button"
                  onClick={() => item.label === "Overview" && router.push("/786.chat")}
                  className={`mb-1 flex h-10 w-full items-center rounded-lg py-2.5 text-[12px] transition ${
          ...
          {!sidebarCollapsed && (
            <p className="mb-2 mt-7 px-2 text-[9px] font-bold uppercase tracking-[.16em] text-slate-600">Support</p>
          )}
          {[
            ["Logs", Logs],
            ["Help & Docs", LifeBuoy],
            ...
            return (
              <button key={String(label)} type="button" className={`flex w-full items-center rounded-lg py-2.5 text-[12px] text-slate-400 hover:bg-white/[.04] ${sidebarCollapsed ? "justify-center" : "gap-3 px-2"}`}>
        ...
        <button type="button" onClick={startNewProject} className="m-2 flex items-center gap-3 rounded-xl border border-[#24324d] bg-[#10172a] p-2 text-left">
          ...
          {!sidebarCollapsed && <span><b className="block text-[11px]">New project</b><span className="text-[9px] text-slate-500">Start clean workspace</span></span>}
        </button>
      </aside>
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col">
        <header className="flex h-[58px] shrink-0 items-center border-b border-[#1b2940] bg-[#070c18]/95 px-4">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <p className="truncate text-[12px] font-bold">{project?.title || "Untitled application"}</p>
            {project && <span className="rounded-full border border-violet-400/30 bg-violet-500/10 px-2 py-0.5 text-[8px] uppercase text-violet-300">Live project</span>}
            <ChevronDown className="h-3 w-3 text-slate-500" />
          </div>
          <div className="mx-auto flex items-center gap-2">
            <span className="rounded-lg border border-violet-400/15 bg-violet-500/10 px-3 py-1.5 text-[9px] font-semibold text-violet-200">
              ✦ {project ? "Design generated" : "Ready to analyse"}
            </span>
            <span className={`rounded-lg border px-3 py-1.5 text-[9px] font-semibold ${build?.status === "failed" ? "border-rose-400/20 bg-rose-500/10 text-rose-200" : "border-emerald-400/15 bg-emerald-500/10 text-emerald-200"}`}>
              {build?.status === "passed" ? "✓ Build passed" : build ? `○ Build ${build.status}` : "○ Build not queued"}
            </span>
          </div>
          <button type="button" onClick={() => setShowCode((value) => !value)} className={`mr-2 inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-[10px] font-bold ${showCode ? "border-violet-300/30 bg-violet-400/15" : "border-[#263550] bg-[#0d1526]"}`}>
            <Code2 className="h-3.5 w-3.5 text-cyan-300" /> Code
          </button>
          <button data-786-publish type="button" disabled={!project || build?.status !== "passed"} className="inline-flex h-9 items-center gap-3 rounded-lg bg-gradient-to-r from-amber-200 to-amber-400 px-5 text-[12px] font-black text-slate-950 shadow-[0_0_22px_rgba(251,191,36,.16)] disabled:opacity-40">
        ...
        <div className="flex min-h-0 flex-1">
          <section style={{ width: agentWidth }} className="relative flex shrink-0 border-r border-[#1b2940] bg-[#080e1c]/90">
            <div className="flex w-[180px] shrink-0 flex-col border-r border-[#1b2940] px-4 py-5">
              <p className="mb-8 flex items-center gap-2 text-[11px] font-bold text-violet-200"><Sparkles className="h-3.5 w-3.5" /> AI Agent</p>
                ...
                {stages.map((stage, index) => {
                  ...
                  return (
                    <div key={stage.label} className="relative mb-8 flex gap-3">
                      ...
                      <div className="pt-2">
                        <p className={`text-[11px] font-bold ${active ? "text-white" : "text-slate-500"}`}><span className="mr-2 text-slate-500">{index + 1}</span>{stage.label}</p>
                        <p className="mt-1 text-[8px] leading-3 text-slate-600">{stage.detail}</p>
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex h-10 items-center border-b border-[#1b2940] px-3 text-[11px] font-bold"><Sparkles className="mr-2 h-3.5 w-3.5 text-violet-300" />Agent Flow</div>
              <div className="min-h-0 flex-1 overflow-y-auto p-3">
                {messages.length === 0 ? (
                  <div className="rounded-xl border border-[#24324d] bg-[#10182b] p-4">
                    <p className="text-[10px] font-bold text-violet-200">Start with a clear application brief</p>
                    <p className="mt-2 text-[10px] leading-5 text-slate-400">Describe the application, pages, users, interactions, backend needs and visual direction. 786.Chat will analyse, plan, generate, validate and build it.</p>
                  </div>
                ) : (
                  messages.map((message) => (
                    <article key={message.id} className={`mb-2 rounded-xl border p-3 ${message.role === "user" ? "border-[#2b3b5d] bg-[#111a2e]" : "border-violet-400/20 bg-violet-500/[.07]"}`}>
                      <p className="mb-2 text-[9px] font-bold text-slate-300">{message.role === "user" ? "● You" : "✦ AI Agent"}</p>
                      <p className="whitespace-pre-wrap text-[10px] leading-5 text-slate-400">{message.content}</p>
                    </article>
                  ))
                )}

                <div className="mt-2 overflow-hidden rounded-xl border border-[#263550] bg-[#0b1221]">
                  <div className="border-b border-[#263550] px-3 py-2 text-[9px] font-bold">Implementation plan</div>
                  {planItems(project).map(([title, detail], index) => (
                    <div key={title} className="flex items-center gap-2 border-b border-[#1d2a41] px-3 py-2.5 last:border-0">
                      ...
                      <span className="min-w-0 flex-1"><b className="block text-[9px]">{title}</b><span className="text-[7px] text-slate-600">{detail}</span></span>
              ...
              <div className="border-t border-[#1b2940] p-2">
                {error && <p className="mb-1 rounded-lg bg-rose-500/10 px-2 py-1 text-[9px] text-rose-200">{error}</p>}
                <div className="flex items-center rounded-lg border border-[#263550] bg-[#0c1424] px-2">
                  ...
                  <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send() } }} rows={1} placeholder="Ask the agent anything…" className="min-h-10 flex-1 resize-none bg-transparent px-2 py-3 text-[10px] outline-none placeholder:text-slate-600" />
                ...
                <div className="mt-2 grid grid-cols-4 gap-1 text-center text-[8px] text-slate-500">
          ...
          <section className="flex min-w-0 flex-1 flex-col bg-[#060b16]">
            <div className="flex h-10 items-center border-b border-[#1b2940] px-3">
              <span className="text-[11px] font-bold">{showCode ? "Project code" : "Live preview"}</span>
              <div className="relative ml-auto">
                {!showCode && (
                  <button type="button" onClick={() => setDeviceOpen((value) => !value)} className="inline-flex h-7 items-center gap-2 rounded-md border border-[#263550] bg-[#0c1424] px-3 text-[8px] font-bold text-slate-300">
                ...
                {deviceOpen && (
                  <div className="absolute right-0 top-9 z-40 w-52 rounded-xl border border-[#263550] bg-[#0b1020] p-2 shadow-2xl">
                    {(Object.keys(BUILDER_DEVICES) as BuilderDevice[]).map((key) => (
                      <button key={key} type="button" onClick={() => { setDevice(key); setDeviceOpen(false) }} className="block w-full rounded-lg px-3 py-2 text-left text-[10px] text-slate-300 hover:bg-white/10">{BUILDER_DEVICES[key].label}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {showCode ? (
                <div className="grid h-full grid-cols-[220px_1fr] overflow-hidden rounded-lg border border-[#263550] bg-[#07101d]">
                  <div className="overflow-auto border-r border-[#263550] p-2">
                    {files.length === 0 && <p className="p-2 text-[9px] text-slate-600">No project files yet.</p>}
                    {files.map((file) => (
                      <button key={file} type="button" onClick={() => setSelectedFile(file)} className={`mb-1 flex w-full items-center gap-2 rounded px-2 py-2 text-left text-[9px] ${selectedFile === file ? "bg-violet-400/15 text-violet-100" : "text-slate-500"}`}>
                        <FileCode2 className="h-3 w-3" /><span className="truncate">{file}</span>
                      </button>
                    ))}
                  </div>
                  <pre className="overflow-auto p-4 text-[10px] leading-5 text-cyan-50"><code>{project?.files[selectedFile] || "No project files yet."}</code></pre>
                </div>
              ) : (
                  ...
                  {build?.status === "passed" && build.deployment_url ? (
                    ...
                    <div style={{ width: deviceSpec.width || "100%", height: deviceSpec.height || "100%", maxWidth: "100%" }} className="grid min-h-full place-items-center rounded-md border border-[#1f2d45] bg-[radial-gradient(circle_at_50%_30%,rgba(30,64,175,.10),transparent_38%),#08111f] px-6 text-center">
                      <div>
                        ...
                        <p className="mt-2 max-w-sm text-[10px] leading-5 text-slate-500">{build?.error_message || (build ? "Preview becomes available only after the isolated Next.js build passes." : "Describe the production application you want to create.")}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {!bottomCollapsed && (
            <div className="grid h-full grid-cols-[.86fr_1.14fr] gap-2 p-2">
              <article className="rounded-lg border border-[#263550] bg-[#0a1120] p-3">
                <div className="flex items-center"><b className="text-[10px]">Build sandbox</b><span className="ml-auto rounded border border-[#263550] px-2 py-1 text-[7px] text-slate-500">Isolated environment</span></div>
                <div className="mt-3 flex h-[112px] items-center rounded-lg border border-dashed border-[#263550] px-4">
                  ...
                  <div><b className="text-[9px]">{build ? `Build ${build.status}` : "No build has run"}</b><p className="mt-1 text-[7px] text-slate-600">{build?.error_message || "The isolated build sandbox starts after validated files are saved."}</p></div>
                </div>
              </article>
              <article className="rounded-lg border border-[#263550] bg-[#0a1120] p-3">
                <div className="flex items-center"><b className="text-[10px]">Revisions</b><span className="ml-auto rounded bg-white/[.04] px-2 py-1 text-[7px] text-slate-500">{project ? "Revision history enabled" : "No project"}</span></div>
                <div className="mt-3 flex h-[112px] items-center rounded-lg border border-dashed border-[#263550] px-4">
                  ...
                  <div><b className="text-[9px]">{project ? "Project revisions are saved automatically" : "No saved revisions"}</b><p className="mt-1 text-[7px] text-slate-600">{project ? "A restore point is created before generated edits and automatic repairs." : "Create a project to begin revision history."}</p></div>
                </div>
              </article>
            </div>
          )}
          <button type="button" onClick={() => setBottomCollapsed((value) => !value)} className="absolute bottom-1 left-1/2 z-40 -translate-x-1/2 rounded-full border border-blue-300/30 bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-1.5 text-[10px] font-bold shadow-[0_0_24px_rgba(59,130,246,.32)]">
