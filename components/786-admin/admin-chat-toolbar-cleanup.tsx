...
function forceCodePanelDark() {
  const section = document.querySelector<HTMLElement>("main > div > section:last-of-type")
  const codeGrid = section?.querySelector<HTMLElement>(":scope > div.grid")
  if (!codeGrid) return
  const fileList = codeGrid.firstElementChild as HTMLElement | null
  const codePane = codeGrid.lastElementChild as HTMLElement | null
  for (const element of [fileList, codePane]) {
    if (!element) continue
    element.style.width = ""
...
export function AdminChatToolbarCleanup() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      main > div > section:last-of-type > div.grid{display:grid!important;grid-template-columns:260px minmax(0,1fr)!important;gap:16px!important;padding:24px!important;min-height:0!important;background:#070b12!important}
      main > div > section:last-of-type > div.grid > div:first-child{width:auto!important;height:auto!important;max-width:none!important;max-height:none!important;min-height:0!important;overflow:auto!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:24px!important;background:#0d1320!important;box-shadow:none!important;padding:12px!important;color:#dbeafe!important}
      main > div > section:last-of-type > div.grid > div:first-child button{background:rgba(255,255,255,.055)!important;color:#cbd5e1!important;border:0!important;border-radius:16px!important;width:100%!important;height:auto!important;box-shadow:none!important}
      main > div > section:last-of-type > div.grid > div:first-child button[class*="bg-cyan"]{background:#67e8f9!important;color:#020617!important}
      main > div > section:last-of-type > div.grid > pre{width:auto!important;height:auto!important;max-width:none!important;max-height:none!important;min-height:0!important;overflow:auto!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:24px!important;background:#0d1320!important;box-shadow:none!important;color:#ecfeff!important}
    `
    document.head.appendChild(style)
    const onModeClick = (event: MouseEvent) => {
      ...
      if (label === "code" || label.includes("code")) {
        forceCodePanelDark()
        window.setTimeout(forceCodePanelDark, 80)
