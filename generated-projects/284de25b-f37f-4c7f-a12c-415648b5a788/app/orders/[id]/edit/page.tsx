...
async function updateOrder(formData: FormData) {
  ...
  const ownedCustomer = (await sql`
    SELECT id FROM customers
    WHERE id = ${customer_id} AND user_id = ${currentUser.id}
    LIMIT 1
  `) as unknown as Array<Record<string, any>>;

  if (ownedCustomer.length === 0) return;
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <form action={updateOrder} className="space-y-4 rounded-lg bg-white p-6 shadow">
          ...
          <div>
            ...
            <input type="number" name="total" step="0.01" min="0" defaultValue={order.total} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
          </div>
