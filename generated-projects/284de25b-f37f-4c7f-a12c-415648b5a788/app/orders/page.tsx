...
export default async function OrdersPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-navy-800 text-white p-4 flex justify-between items-center">
              ...
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full ${statusClass(order.status)}`}>{order.status}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <form action={`/api/orders/${order.id}`} method="POST" className="inline">
