...
export default async function AdminEditPage({ params }: { params: { id: string } }) {
  ...
  // Try order
  const orders = (await sql`SELECT id, customer_id, status, total FROM orders WHERE id = ${id} AND user_id = ${user.id}`) as unknown as Array<Record<string, any>>;
  if (orders.length > 0) {
    const order = orders[0];
    return <AdminEditForm type="order" initialData={order} />;
  }
