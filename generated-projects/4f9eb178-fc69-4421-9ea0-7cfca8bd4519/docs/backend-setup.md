...
### Stock Movements
- `GET /api/stock-movements` - List stock movements with filters and pagination (includes batch info, supports WASTAGE type)
...
### Wastage
- `POST /api/wastage` - Record wastage (validates staff PIN, reduces stock and batch quantities, creates wastage record and stock movement in a transaction)

### Staff
- `GET /api/staff` - List staff (id, name)
