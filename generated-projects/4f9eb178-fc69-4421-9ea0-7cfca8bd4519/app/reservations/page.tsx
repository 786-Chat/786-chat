...
export default function ReservationsPage() {
  ...
  const filteredReservations = useMemo(() => {
    ...
    return reservations.filter((r) => {
      ...
      const date = String(r.booking_date).slice(0, 10);
      const status = r.status.toLowerCase();
      return customerName.includes(term) || date.includes(term) || status.includes(term);
    });
  }, [reservations, search]);
    ...
    <div className="min-w-0 space-y-8 overflow-x-hidden">
      ...
      <div className="card-gold min-w-0 overflow-hidden">
        ...
        <div className="relative mb-4 min-w-0">
          <input className="input w-full min-w-0 pr-10" placeholder="Search by customer name, date, or status..." value={search} onChange={(e) => setSearch(e.target.value)} aria-label="Search reservations" />
          {search && <button type="button" onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-900" aria-label="Clear reservation search">×</button>}
        </div>
        <div className="space-y-3 sm:hidden">
          {filteredReservations.map((r) => <article key={r.id} className="min-w-0 rounded-xl border border-gray-200 bg-white/85 p-4 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => startEdit(r)} className="rounded-lg border border-deepgreen px-3 py-2 text-sm font-medium text-deepgreen">Edit</button>
              {r.status !== 'cancelled' && <button type="button" onClick={() => updateStatus(r, 'cancelled')} className="rounded-lg border border-amber-500 px-3 py-2 text-sm font-medium text-amber-700">Cancel</button>}
              <button type="button" onClick={() => setDeleteTarget(r)} title="Delete reservation" aria-label="Delete reservation" className="ml-auto inline-flex items-center justify-center rounded-lg border border-red-200 bg-red-50 p-2 text-red-600 transition hover:bg-red-100 hover:text-red-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          </article>)}
          {filteredReservations.length === 0 && <p className="py-6 text-center text-gray-500">No reservations found</p>}
        </div>
        <div className="hidden overflow-x-auto sm:block">
          ...
          {filteredReservations.map((r) => <tr key={r.id} className="border-b align-top"><td className="py-2">{r.customer_name || `Customer ${r.customer_id}`}{r.customer_vip && <span className="ml-2 rounded-full bg-gold px-2 py-0.5 text-xs font-bold text-white">VIP</span>}</td><td className="py-2">{formatDate(r.booking_date)}</td><td className="py-2">{String(r.booking_time).slice(0, 5)}</td><td className="py-2">{r.guests}</td><td className="py-2 capitalize">{r.status}</td><td className="py-2"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => startEdit(r)} className="text-sm font-medium text-deepgreen">Edit</button>{r.status !== 'cancelled' && <button type="button" onClick={() => updateStatus(r, 'cancelled')} className="text-sm font-medium text-amber-700">Cancel</button>}<button type="button" onClick={() => setDeleteTarget(r)} title="Delete reservation" aria-label="Delete reservation" className="inline-flex items-center justify-center rounded-lg border border-red-200 bg-red-50 p-2 text-red-600 transition hover:bg-red-100 hover:text-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}
          {filteredReservations.length === 0 && <tr><td colSpan={6} className="py-6 text-center text-gray-500">No reservations found</td></tr>}
          </tbody></table>
        </div>
      </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-600">This reservation will be permanently deleted.</p>
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setDeleteTarget(null)} disabled={deleting} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Keep reservation</button>
