...
export default function ReservationsPage() {
  ...
  const deleteReservation = async (reservation: Reservation) => {
    if (!window.confirm(`Delete reservation for ${reservation.customer_name || `customer ${reservation.customer_id}`}?`)) return;
    setMessage('');
    setError('');
    try {
      const res = await fetch(`/api/reservations/${reservation.id}`, {
    ...
    <div className="min-w-0 space-y-8 overflow-x-hidden">
      ...
      <div className="card-gold min-w-0">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          {editingId && (
            <button type="button" onClick={resetForm} className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium hover:bg-white/70">
              Cancel edit
            </button>
          )}
        </div>
        <form onSubmit={handleSubmit} className="min-w-0 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select className="input w-full min-w-0" value={status} onChange={(e) => setStatus(e.target.value as Reservation['status'])}>
      ...
      <div className="card-gold min-w-0 overflow-hidden">
        ...
        <div className="space-y-3 sm:hidden">
          {reservations.map((r) => (
            <article key={r.id} className="min-w-0 rounded-xl border border-gray-200 bg-white/85 p-4 shadow-sm">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                <h3 className="min-w-0 break-words font-semibold text-gray-900">{r.customer_name || `Customer ${r.customer_id}`}</h3>
                {r.customer_vip && <span className="shrink-0 rounded-full bg-gold px-2 py-0.5 text-xs font-bold text-white">VIP</span>}
              </div>
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <div><dt className="text-gray-500">Date</dt><dd className="font-medium text-gray-900">{formatDate(r.booking_date)}</dd></div>
                <div><dt className="text-gray-500">Time</dt><dd className="font-medium text-gray-900">{String(r.booking_time).slice(0, 5)}</dd></div>
                <div><dt className="text-gray-500">Guests</dt><dd className="font-medium text-gray-900">{r.guests}</dd></div>
                <div><dt className="text-gray-500">Status</dt><dd className="break-words font-medium capitalize text-gray-900">{r.status}</dd></div>
              </dl>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => deleteReservation(r)} className="rounded-lg border border-red-500 px-3 py-2 text-sm font-medium text-red-600">Delete</button>
              </div>
            </article>
          ))}
        ...
        <div className="hidden overflow-x-auto sm:block">
          <table className="w-full text-left">
            <thead><tr className="border-b"><th className="py-2">Customer</th><th className="py-2">Date</th><th className="py-2">Time</th><th className="py-2">Guests</th><th className="py-2">Status</th><th className="py-2">Actions</th></tr></thead>
            <tbody>
              {reservations.map((r) => (
                <tr key={r.id} className="border-b align-top">
                  <td className="py-2">{r.customer_name || `Customer ${r.customer_id}`}{r.customer_vip && <span className="ml-2 rounded-full bg-gold px-2 py-0.5 text-xs font-bold text-white">VIP</span>}</td>
                  <td className="py-2">{formatDate(r.booking_date)}</td><td className="py-2">{String(r.booking_time).slice(0, 5)}</td><td className="py-2">{r.guests}</td><td className="py-2 capitalize">{r.status}</td>
                  <td className="py-2"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => startEdit(r)} className="text-sm font-medium text-deepgreen">Edit</button>{r.status !== 'cancelled' && <button type="button" onClick={() => updateStatus(r, 'cancelled')} className="text-sm font-medium text-amber-700">Cancel</button>}<button type="button" onClick={() => deleteReservation(r)} className="text-sm font-medium text-red-600">Delete</button></div></td>
                </tr>
              ))}
