...
export default function CustomersPage() {
  ...
  return (
    <div className="min-w-0 space-y-8 overflow-x-hidden relative">
      ...
      <div className="card-blue min-w-0">
      ...
      <div className="card-blue min-w-0 overflow-hidden">
        ...
        <div className="space-y-3 sm:hidden">
          {filteredCustomers.map((c) => (
            <article key={c.id} className="min-w-0 rounded-xl border border-gray-200 bg-white/80 p-4 shadow-sm">
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <h3 className="min-w-0 break-words font-semibold text-gray-900">{c.name}</h3>
                    {c.vip && <span className="shrink-0 rounded-full bg-gold px-2 py-0.5 text-xs font-bold text-white">VIP</span>}
                  </div>
            ...
            <tbody>
              {filteredCustomers.map((c) => (
                <tr key={c.id} className="border-b">
                  <td className="py-2">{c.name}{c.vip && <span className="ml-2 bg-gold text-white text-xs font-bold px-2 py-0.5 rounded-full">VIP</span>}</td>
