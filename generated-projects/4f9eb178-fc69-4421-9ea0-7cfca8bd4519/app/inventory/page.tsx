          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="border-b">
                <th className="py-3 pr-6 text-sm font-semibold text-gray-700">Product</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">Cost</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">Price ex VAT</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">VAT %</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">VAT Amount</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">Total inc VAT</th>
                <th className="py-3 pr-6 text-right text-sm font-semibold text-gray-700">Stock</th>
                <th className="py-3 text-sm font-semibold text-gray-700">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredProducts.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-gray-100 align-middle">
                    ...
                    <td className="py-3 pr-6 text-right tabular-nums text-gray-900">{formatGBP(p.cost_price)}</td>
                    <td className="py-3 pr-6 text-right tabular-nums text-gray-900">{formatGBP(sellingPrice)}</td>
                    <td className="py-3 pr-6 text-right tabular-nums text-gray-900">{vatPercent}%</td>
                    <td className="py-3 pr-6 text-right tabular-nums text-gray-900">{formatGBP(vat)}</td>
                    <td className="py-3 pr-6 text-right tabular-nums font-semibold text-gray-900">{formatGBP(total)}</td>
                    <td className="py-3 pr-6 text-right tabular-nums text-gray-900">{p.stock_quantity} {p.unit}</td>
                    <td className="py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <button type="button" onClick={() => startEdit(p)} className="text-sm font-medium text-deepgreen">Edit</button>
                        <button type="button" onClick={() => openStockIn(p)} className="inline-flex items-center gap-1 rounded-lg border border-deepgreen bg-deepgreen px-2 py-1 text-sm font-medium text-white"><PackagePlus className="h-4 w-4" />Stock In</button>
