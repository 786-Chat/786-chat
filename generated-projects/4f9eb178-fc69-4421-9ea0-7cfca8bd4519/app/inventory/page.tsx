            ...
            {filteredProducts.map((p) => {
              ...
              return (
                <article key={p.id} className="flex items-stretch gap-6 rounded-xl border border-gray-200 bg-white/85 p-6 shadow-sm">
                  {/* Area 1: Product details */}
                  <div className="flex min-w-0 flex-1 flex-col justify-center border-r border-gray-200 pr-6">
                  ...
                  {/* Area 2: Product values */}
                  <div className="flex min-w-0 flex-1 flex-col justify-center border-r border-gray-200 pr-6">
                    <dl className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                      <div className="flex justify-between gap-2"><dt className="text-gray-500">Cost</dt><dd className="font-medium text-gray-900">{formatGBP(p.cost_price)}</dd></div>
                      <div className="flex justify-between gap-2"><dt className="text-gray-500">Price ex VAT</dt><dd className="font-medium text-gray-900">{formatGBP(sellingPrice)}</dd></div>
                      <div className="flex justify-between gap-2"><dt className="text-gray-500">VAT</dt><dd className="font-medium text-gray-900">{vatPercent}%</dd></div>
                      <div className="flex justify-between gap-2"><dt className="text-gray-500">VAT Amount</dt><dd className="font-medium text-gray-900">{formatGBP(vat)}</dd></div>
                      <div className="flex justify-between gap-2"><dt className="text-gray-500">Total inc VAT</dt><dd className="font-semibold text-gray-900">{formatGBP(total)}</dd></div>
                    </dl>
                  </div>
                  ...
                  <div className="flex min-w-0 flex-1 flex-col justify-center border-r border-gray-200 pr-6">
                    ...
                    {/* Future low-stock badge placeholder */}
                  </div>

                  {/* Area 4: Actions */}
                  <div className="flex min-w-0 flex-1 flex-col items-start justify-center gap-2">
                    <button type="button" onClick={() => startEdit(p)} className="rounded-lg border border-deepgreen px-4 py-2 text-sm font-medium text-deepgreen">Edit</button>
                    <button type="button" onClick={() => openStockIn(p)} className="inline-flex items-center gap-1 rounded-lg border border-deepgreen bg-deepgreen px-4 py-2 text-sm font-medium text-white"><PackagePlus className="h-4 w-4" />Stock In</button>
