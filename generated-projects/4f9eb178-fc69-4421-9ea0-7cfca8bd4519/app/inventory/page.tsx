  ...
  const submitStockIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/products/${stockInTarget.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', ...tenantHeaders },
        body: JSON.stringify({ stock_quantity: newStock, last_stock_in_at: new Date().toISOString() }),
      });
      const data = await res.json();
  ...
  const submitStockOut = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/products/${stockOutTarget.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', ...tenantHeaders },
        body: JSON.stringify({ stock_quantity: newStock, last_stock_out_at: new Date().toISOString() }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to update stock');
      await loadProducts();
