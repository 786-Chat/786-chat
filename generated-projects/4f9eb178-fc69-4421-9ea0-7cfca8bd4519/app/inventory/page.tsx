            ...
            return (
              <article key={p.id} className={`min-w-0 rounded-xl border bg-white/85 p-4 shadow-sm ${cardBorderClass(p)}`}>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <button type="button" onClick={() => openStockOut(p)} disabled={p.stock_quantity === 0} className="inline-flex items-center gap-1 rounded-lg border border-amber-500 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700 disabled:cursor-not-allowed disabled:opacity-50"><PackageMinus className="h-4 w-4" />Stock Out</button>
                  <button type="button" onClick={() => openDetails(p)} className="inline-flex items-center gap-1 rounded-lg border border-blue-500 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700"><Info className="h-4 w-4" />Stock Details</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={submitDetails} className="mt-6 space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div><label className="label">Used Date</label><input type="date" className="input w-full" value={detailsForm.used_date} onChange={(e) => setDetailsForm({ ...detailsForm, used_date: e.target.value })} /></div>
                <div><label className="label">Best Before Date</label><input type="date" className="input w-full" value={detailsForm.best_before_date} onChange={(e) => setDetailsForm({ ...detailsForm, best_before_date: e.target.value })} /></div>
                <div><label className="label">Use By Date</label><input type="date" className="input w-full" value={detailsForm.use_by_date} onChange={(e) => setDetailsForm({ ...detailsForm, use_by_date: e.target.value })} /></div>
                <div><label className="label">Expiry Date</label><input type="date" className="input w-full" value={detailsForm.expiry_date} onChange={(e) => setDetailsForm({ ...detailsForm, expiry_date: e.target.value })} /></div>
              </div>
              {error && <p className="break-words text-red-600">{error}</p>}
              <div className="flex justify-end gap-3">
