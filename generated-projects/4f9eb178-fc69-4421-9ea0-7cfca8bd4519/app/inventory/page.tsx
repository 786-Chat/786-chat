          ...
          <div className="flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-xl bg-white p-6 shadow-xl">
            <div className="overflow-y-auto pr-1">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex justify-between text-sm"><span className="text-gray-500">Rotation Method</span><span className="font-medium text-gray-900">{detailsTarget.rotation_method}</span></div>
              </div>
              <div className="mt-6">
                <h4 className="text-md font-semibold text-gray-900">Active Batches</h4>
                <div className="mt-2 space-y-2">
                  {activeBatches.length === 0 && <p className="text-sm text-gray-500">No active batches</p>}
                  {activeBatches.map((b) => (
                    <div key={b.id} className="rounded-lg border border-gray-200 p-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="font-medium text-gray-900">{b.batch_number}</span>
                        <span className="rounded-full bg-green-600 px-2 py-0.5 text-xs font-bold text-white">{b.status}</span>
                      </div>
                      <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                        <div><span className="text-gray-500">Stock In:</span> <span className="font-medium">{formatDate(b.stock_in_date)}</span></div>
                        <div><span className="text-gray-500">Remaining:</span> <span className="font-medium">{b.qty_remaining} {b.unit}</span></div>
