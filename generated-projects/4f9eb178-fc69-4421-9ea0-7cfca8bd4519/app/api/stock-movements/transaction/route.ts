...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { product_id, movement_type, qty, reference, user_id, user_name, batch_number, batch_id, best_before_date, use_by_date, expiry_date } = body;
    ...
    if (movement_type === 'STOCK IN') {
      ...
      const batchResult = await query(
        ...
        [batchNumber, product_id, product.sku, product.product_name, qtyNum, product.unit, stockInDate, stockInTime, reference || '', product.supplier || '', best_before_date || null, use_by_date || null, expiry_date || null, user_name]
      );
