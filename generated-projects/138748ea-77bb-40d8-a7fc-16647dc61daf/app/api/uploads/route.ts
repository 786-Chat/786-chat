...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = (await sql`
      ...
      VALUES (${user.userId}, ${tenantId}, ${blobPath}, ${blob.url}, ${parsed.data.fileName}, ${parsed.data.mimeType}, ${parsed.data.sizeBytes})
      RETURNING id, blob_path, blob_url, file_name, mime_type, size_bytes, created_at
    `) as unknown as Array<Record<string, any>>;

    await sql`INSERT INTO audit_logs (user_id, company_id, action, entity_type, entity_id, metadata) VALUES (${user.userId}, ${tenantId}, 'upload', 'uploads', ${result[0].id}, ${JSON.stringify({ fileName: parsed.data.fileName })})`;
