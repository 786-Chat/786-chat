...
export async function refreshAssignedDevice(storageInstance: any, dev: any, options: { notify?: boolean } = {}): Promise<any> {
  ...
  // Latch the alarm until Admin/branch acknowledgement, even when a brief sensor pulse returns to normal.
  const nextAlarmActive = Boolean(dev.alarmActive || alarmDetected);
