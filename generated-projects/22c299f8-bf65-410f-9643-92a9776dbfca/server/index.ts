...
app.use('/attached_assets', express.static(path.join(process.env.TMPDIR || "/tmp", "attached_assets"), {
  maxAge: '365d', // Cache for 1 year (24-month policy)
  setHeaders: (res, filePath) => {
    // Set cache headers for better performance and 24-month retention
    res.setHeader('Cache-Control', 'public, max-age=31536000'); // 1 year
...
app.use('/data', express.static(path.join(process.env.TMPDIR || "/tmp", "data"), {
