...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const contractNumberForSlot = (slotNumber: number) => `UN5-163-${String(slotNumber).padStart(4, "0")}`;

  const normalizeBranchForAdmin = (branch: Branch) => {
    const settings = (branch.settings as any) || {};
    const slotNumber = Number(settings.slotNumber || 0);
    const legacyPlaceholder = String(branch.name || "").trim().startsWith("Available Branch ");
    return {
      ...branch,
      name: legacyPlaceholder ? "" : branch.name,
      contractNumber: branch.contractNumber || (slotNumber > 0 ? contractNumberForSlot(slotNumber) : undefined),
      password: undefined
    };
  };
  ...
  app.get('/api/branches', isAdminAuthenticated, async (req, res) => {
    try {
      ...
      res.json({
        branches: branches.map(normalizeBranchForAdmin),
        pagination: {
          currentPage: page,
          totalPages,
          totalCount,
  ...
  app.post('/api/branches', isAdminAuthenticated, uploadLogoSafe, async (req, res) => {
    ...
    try {
      ...
      if (!String(branchData.contractNumber || "").trim()) branchData.contractNumber = "UN5-163-26";
      ...
      if (uploadedLogo) {
        try {
          ...
          if (uploadedLogo.path) {
            ...
            if (!String(updateData.contractNumber || "").trim()) updateData.contractNumber = "UN5-163-26";
  ...
  app.post('/api/admin/bulk-create-branches', isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const slots = Array.from({ length: missingCount }, (_, index) => {
        ...
        return {
          name: "",
          contractNumber: contractNumberForSlot(slotNumber),
          status: 'inactive',
  ...
  // Send photo to branch - reuse the permanent stored image without downloading it again
  app.post('/api/photos/:id/send', isAdminAuthenticated, async (req, res) => {
    ...
    try {
      ...
      const branch = await storage.getBranch(branchId);
      if (!branch || !String(branch.name || '').trim()) {
        return res.status(400).json({ message: "Please select a branch with a business name" });
      }
      ...
      // Assign the existing permanent photo record to the selected branch.
      // Do not create a duplicate admin photo card.
      const branchPhoto = await storage.updatePhoto(photoId, {
        branchId,
        sentAt: new Date(),
      } as any);
      ...
      const duplicateExists = existingNotifications.some(notification =>
        notification.message === notificationMessage && notification.visitDate === today
      );
      ...
      res.json({
        success: true,
        message: `Photo sent to ${branch.name} successfully`,
  ...
  // Payment Records API  
  app.get("/api/payment-records", isAdminAuthenticated, (req: any, res: any) => {
  ...
  app.post("/api/payment-records", isAdminAuthenticated, (req: any, res: any) => {
  ...
  app.delete("/api/payment-records/:id", isAdminAuthenticated, (req: any, res: any) => {
...
- Lifetime document storage guarantee
  ...
  app.get("/api/tuya/test-connection", isAdminAuthenticated, async (_req, res) => {
    try {
      res.json(await tuyaService.testConnection());
    } catch (err: any) {
  ...
  app.get("/api/tuya/devices", isAdminAuthenticated, async (_req, res) => {
    try {
      ...
      const enriched = await Promise.all(devices.map(async (device: any) => {
        const id = String(device.id || device.device_id || device.deviceId || "");
        if (!id) return device;
        try {
          const [info, status] = await Promise.all([
            tuyaService.getDeviceInfo(id).catch(() => null),
            tuyaService.getDeviceStatus(id).catch(() => []),
          ]);
          return {
            ...device,
            ...info,
            id,
            online: info?.online ?? info?.is_online ?? device?.online ?? false,
            status,
            batteryPercent: tuyaService.getBatteryPercent(status),
            powerStatus: tuyaService.getPowerStatus(status),
            alarmActive: tuyaService.checkAlarmActive(status),
          };
        } catch (_) {
          return { ...device, id };
        }
  ...
  app.get("/api/tuya/devices/:deviceId", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const status = await tuyaService.getDeviceStatus(req.params.deviceId).catch(() => []);
      res.json({
        ...info,
        status,
        batteryPercent: tuyaService.getBatteryPercent(status),
      ...
      res.status(500).json({ message: err?.message || "Unable to load device" });
    }
  });

  app.get("/api/tuya/devices/:deviceId/status", isAdminAuthenticated, async (req, res) => {
    try {
      res.json(await tuyaService.getDeviceStatus(req.params.deviceId));
    } catch (err: any) {
      res.status(500).json({ message: err?.message || "Unable to load device status" });
    }
  });
  ...
  app.post("/api/tuya/devices/:deviceId/commands", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      if (!commands.length) return res.status(400).json({ message: "commands are required" });
      res.json(await tuyaService.sendDeviceCommands(req.params.deviceId, commands));
    } catch (err: any) {
      res.status(500).json({ message: err?.message || "Unable to send command" });
    }
  });
  ...
  app.post("/api/iot/devices", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      if (!deviceId || !String(deviceName || "").trim() || !branchId) {
        return res.status(400).json({ message: "Cloud device, friendly name, and branch are required" });
      }
      ...
      const device = await storage.createIotDevice({
        ...
        alarmActive: tuyaService.checkAlarmActive(statusList),
        lastStatus: statusList,
        lastAlarmAt: tuyaService.checkAlarmActive(statusList) ? new Date() : null,
        lastCheckedAt: new Date(),
      } as any);
      res.json(device);
    } catch (err: any) {
      res.status(500).json({ message: err?.message || "Unable to assign device" });
    }
  });
  ...
  app.get("/api/branch/iot-devices", async (req, res) => {
    try {
      ...
      let devices = await storage.getIotDevicesByBranch(session.branchId);
      if (String(req.query.refresh || "") === "1" && tuyaService.isConfigured()) {
        devices = await Promise.all(devices.map(async (device: any) => {
          try { return await tuyaService.refreshAssignedDevice(storage, device, { notify: true }); }
          catch (_) { return device; }
