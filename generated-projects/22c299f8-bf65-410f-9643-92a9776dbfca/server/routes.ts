...
// 6. Branch Logos Section - Admin Dashboard
const logoUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      const branchId = req.params.id || (req.session as any)?.branchId;
      const timestamp = Date.now();
  ...
  fileFilter: (req, file, cb) => {
    // Enhanced support for ALL image formats with lifetime display
    const allowedTypes = [
      'image/jpeg', 
      'image/jpg', 
      'image/png', 
    ...
    const allowedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.webp', '.bmp', '.tiff'];
    
    if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(fileExtension)) {
      console.log(`✅ LOGO FORMAT ACCEPTED: ${file.originalname} (${file.mimetype}) - LIFETIME DISPLAY ENABLED`);
      cb(null, true);
    } else {
      console.log(`❌ LOGO FORMAT REJECTED: ${file.originalname} (${file.mimetype})`);
      cb(new Error('Invalid file type. Only JPG, JPEG, PNG, GIF, SVG, WebP, BMP, and TIFF files are allowed for logos with lifetime display.'));
    }
  },
  limits: {
    fileSize: 50 * 1024 * 1024 // 50MB limit (animated GIFs can be large)
  }
});
    ...
    if (err) {
      if (err.code === 'LIMIT_FILE_SIZE') {
        return res.status(400).json({
          message: 'Logo file is too large. Maximum size is 50MB. Please compress your GIF or use a smaller image.'
        });
      }
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/verify-login", (req, res) => {
    try {
      ...
      if (isAdminCredentials || isMasterPassword || isTemporaryPin) {
        ...
        // Set admin token directly without touching sessions (to avoid staff access conflicts)
        ...
        console.log(`Admin login successful for: ${loginField}, token: ${token}`);
        res.json({ success: true, message: "Admin login successful", token, user: loginField });
      } else {
        console.log(`Login failed for: ${loginField}, password: ${password}`);
  ...
  app.get("/api/admin/auth-check", (req, res) => {
    try {
      const token = req.cookies?.admin_token;
      console.log(`Admin auth check - token: ${token}, valid: ${token && adminTokens.has(token)}`);
      ...
      if (token && !adminTokens.has(token)) {
        ...
        if (token.startsWith('admin_') && token.includes('_')) {
          ...
          if (parts.length === 3) {
            ...
            const now = Date.now();
            // Accept tokens that are less than 24 hours old
            if (now - timestamp < 24 * 60 * 60 * 1000) {
              adminTokens.add(token);
              console.log(`Re-added valid admin token: ${token}`);
  ...
  const isAdminAuthenticated = (req: any, res: any, next: any) => {
    const token = req.cookies?.admin_token;
    console.log(`🔒 Admin auth check - URL: ${req.url}, Method: ${req.method}, token: ${token}, valid: ${token && adminTokens.has(token)}`);
  ...
  app.post('/api/branches', isAdminAuthenticated, uploadLogoSafe, async (req, res) => {
    try {
      ...
      // Add logo URL if file was uploaded - use Object Storage for permanent persistence
      if (req.file) {
        console.log('📁 Logo file uploaded:', req.file.filename);
        
        // Get temporary branchId from request (will be overridden by database after creation)
        const tempBranchId = req.body.id || 'temp-' + Date.now();
        ...
        branchData.logoUrl = logoUrl;
        console.log(`✅ Logo uploaded to Object Storage for new branch: ${logoUrl}`);
      ...
      console.log('✅ Branch data validated successfully');
  ...
  app.patch('/api/branches/:id', isAdminAuthenticated, uploadLogoSafe, async (req, res) => {
    try {
      ...
      // Convert date strings to Date objects (or null) for timestamp columns
      if ('lastTraining' in updateData) {
        updateData.lastTraining = updateData.lastTraining && updateData.lastTraining !== ''
          ? new Date(updateData.lastTraining)
          : null;
      }
      if ('trainingNextDue' in updateData) {
        updateData.trainingNextDue = updateData.trainingNextDue && updateData.trainingNextDue !== ''
          ? new Date(updateData.trainingNextDue)
          : null;
      }
  ...
  // Development bulk creation endpoint (no auth required for testing)
  app.post('/api/dev/bulk-create-branches', async (req, res) => {
    try {
      const { count = 3000 } = req.body;
      
      if (count > 5000) {
        return res.status(400).json({ message: "Maximum 5000 branches allowed per bulk creation" });
      }

      console.log(`🚀 Starting bulk creation of ${count} branches...`);
      
      // Generate realistic UK branch data
      const branchesData = generateUKBranchData(count);
      console.log(`📝 Generated ${branchesData.length} branch records`);
      
      // Skip validation for bulk creation to handle date format issues
      console.log(`📋 Bypassing validation for bulk creation performance`);
      console.log(`📝 Sample branch data:`, JSON.stringify(branchesData[0], null, 2));
      
      // Create branches in bulk
      const createdBranches = await storage.bulkCreateBranches(branchesData);
      ...
      res.json({
        ...
        branches: createdBranches.slice(0, 10).map(branch => ({ ...branch, password: undefined })) // Return first 10 without passwords
      });
      
    } catch (error: any) {
      console.error("❌ Error in bulk branch creation:", error);
      res.status(500).json({ 
        message: "Failed to create branches in bulk",
        error: error.message,
        details: "Check server logs for more information"
      });
    }
  ...
  app.post('/api/admin/bulk-create-branches', isAdminAuthenticated, async (req, res) => {
    try {
      ...
      // Validate the first batch to ensure schema compliance
      const sampleBatch = branchesData.slice(0, 5);
      for (const branch of sampleBatch) {
        try {
          insertBranchSchema.parse(branch);
        } catch (validationError) {
          console.error("Validation error in generated data:", validationError);
