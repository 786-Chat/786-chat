...
export class DatabaseStorage implements IStorage {
  ...
  async createBranch(branch: InsertBranch): Promise<Branch> {
    // Each customer branch is created as an empty, isolated record. Real branch
    // documents/photos/reports are added later and always reference this branch ID.
    const branchData: any = { ...branch };
  ...
  async normalizeReservedBranchSlots(): Promise<number> {
    // Convert only legacy reserved placeholders. Real customer branch names are never changed.
    const result: any = await db.execute(sql`
      ...
      SET
        ...
        settings = jsonb_set(COALESCE(settings, '{}'::jsonb), '{available}', 'true'::jsonb, true),
        updated_at = NOW()
      WHERE name ~* '^Available Branch[[:space:]]+[0-9]+$'
    `);
  ...
  async restoreAllBranchLogos(): Promise<void> {
    ...
    try {
      ...
      const uploadsDir = path.join(process.env.TMPDIR || "/tmp", "uploads");
      const attachedDir = path.join(process.env.TMPDIR || "/tmp", "attached_assets");
