...
async function throwIfResNotOk(res: Response, url?: string) {
  if (!res.ok) {
    ...
    // Do not globally navigate on every 401. Individual authenticated screens
    // own their auth probe and redirect. This prevents an optional API from
    // ejecting a valid admin/branch session.
    if (res.status === 401) {
