...
function BranchLoginVideoManager() {
  ...
  const uploadVideo = async () => {
    ...
    try {
      ...
      const uploadPromise = upload(`pest-control/branch-login/${Date.now()}-${safeName}`, selectedVideo, {
        access: 'public',
        handleUploadUrl: '/api/admin/branch-login-video',
        contentType: selectedVideo.type || 'video/mp4',
        onUploadProgress: ({ percentage }) => setUploadProgress(Math.min(99, Math.round(percentage))),
      }).then(async (blob) => {
      ...
      const publishedPromise = (async () => {
        for (let attempt = 0; attempt < 90; attempt += 1) {
          await new Promise((resolve) => window.setTimeout(resolve, 2000));
          try {
            const response = await fetch(`/api/public/branch-login-media?t=${Date.now()}`, { credentials: 'include', cache: 'no-store' });
            if (!response.ok) continue;
            const data = await response.json();
            const currentUrl = String(data?.videoUrl || '');
            if (currentUrl && currentUrl !== previousUrl) return currentUrl;
          } catch {}
        }
        throw new Error('Upload did not finish. Please try again.');
      })();

      const publishedUrl = await Promise.race([uploadPromise, publishedPromise]);
      setVideoUrl(publishedUrl);
        ...
        <CardContent className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <Button onClick={uploadVideo} disabled={!selectedVideo || uploading} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white">
              ...
              {uploading ? (uploadProgress >= 99 ? 'Finishing upload…' : `Uploading ${uploadProgress}%`) : 'Upload & Publish to Branch Login'}
            </Button>
          </div>
        </CardContent>
      </Card>
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        // Protected dashboard data may load only after the admin session is confirmed.
  ...
  const { data: branchesResponse, isLoading: branchesLoading, error: branchesError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/branches?${params}`, { credentials: 'include' });
  ...
  // Monthly Report send dialog: load every real branch, not reserved Available Branch slots.
  const { data: reportBranchesResponse } = useQuery({
    queryKey: ["/api/branches", "monthly-report-send-options"],
    queryFn: async () => {
      const response = await fetch("/api/branches?page=1&limit=5000", { credentials: 'include' });
        ...
        {reportsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 w-full pb-4">
            {monthlyReports
              ...
              .filter(report => !branchSearchTerm || report.title?.toLowerCase().includes(branchSearchTerm.toLowerCase()) || reportBranchOptions.find((b: Branch) => b.id === report.branchId)?.name?.toLowerCase().includes(branchSearchTerm.toLowerCase()))
              .map((report) => (
                  ...
                  <CardContent className="p-4 flex flex-col min-h-[270px]">
                    ...
                    <div className="text-center mb-3">
                      <span className="inline-flex px-2 py-1 rounded-full bg-green-500/15 text-green-300 text-xs">
                        {report.branchId ? (reportBranchOptions.find((b: Branch) => b.id === report.branchId)?.name || 'Branch') : 'Not assigned'}
                      </span>
                    </div>
...
{/* Pest Control Documents Section - Mobile Responsive */}
            ...
            {activeTab === "yearly-docs" && (
              <YearlyDocsAdminSection />
            )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} readOnly className="bg-slate-800/50 border-slate-700 text-white cursor-not-allowed" placeholder="Assigned automatically" />
                        </FormControl>
                        <FormMessage className="text-red-400" />
                      </FormItem>
                    )}
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {reportBranchOptions
                    .filter((branch: Branch) => String(branch.name || '').trim())
                    .map((branch: Branch) => (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={selectedBranchForReport} onValueChange={setSelectedBranchForReport}>
                ...
                <SelectContent className="bg-slate-900 border-slate-600 z-[10000] max-h-72 overflow-y-auto">
                  {reportBranchOptions.map((branch: Branch) => (
                    <SelectItem key={branch.id} value={branch.id} className="text-white hover:bg-slate-700 focus:bg-slate-700">
