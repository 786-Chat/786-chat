...
export default function AdminDashboard() {
  ...
  const reAuthenticateAdmin = async () => {
    try {
      // First try with the master password
      ...
      if (result.success) {
        // Server manages authentication via cookies
        return { success: true };
      }
  ...
  const onSubmit = async (data: BranchFormData) => {
    if (selectedBranch) {
      ...
      // Add mode - create branch first, then upload logo if provided
      const newBranchResponse = await addBranchMutation.mutateAsync(data);
      
      // Upload logo if file is selected (fixed variable name)
      if (branchLogoFile && newBranchResponse?.branch?.id) {
        const logoFormData = new FormData();
        logoFormData.append('logo', branchLogoFile);
        
        uploadLogoMutation.mutate({
          branchId: newBranchResponse.branch.id,
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (response.status === 401) {
            ...
            const reAuthResult = await reAuthenticateAdmin();
            if (reAuthResult.success) {
              // Retry the delete operation
              const retryResponse = await fetch(`/api/branches/${branchId}`, {
                method: 'DELETE',
                headers: {
                  'Content-Type': 'application/json',
                },
  ...
  const handleCardLogoUpload = (branchId: string, file: File | undefined) => {
    ...
    const allowedTypes = ['image/png', 'image/jpeg', 'image/jpg', 'image/gif', 'image/svg+xml', 'image/webp', 'image/bmp', 'image/tiff'];
    if (!allowedTypes.includes(file.type)) {
      console.log('❌ Invalid file type:', file.type);
      toast({
        title: "Invalid File Type",
        description: "Please select a valid image file (PNG, JPG, GIF, SVG, WebP, BMP, TIFF)",
        variant: "destructive",
      });
      return;
    }
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept="image/*,.png,.jpg,.jpeg,.gif,.svg,.webp,.bmp,.tiff"
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (file) {
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Supported formats: PNG, JPG, GIF, SVG, WebP, BMP, TIFF. Max size: 10MB
                    </p>
                  </div>
