...
const branchVideos = [
  "https://raw.githubusercontent.com/786-Chat/786-chat/43efbbe87f3aca121164fd775f5cfb1389faf74d/generated-projects/22c299f8-bf65-410f-9643-92a9776dbfca/client/public/branch-video-1.mp4",
  "https://raw.githubusercontent.com/786-Chat/786-chat/43efbbe87f3aca121164fd775f5cfb1389faf74d/generated-projects/22c299f8-bf65-410f-9643-92a9776dbfca/client/public/branch-video-2.mp4",
] as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 flex flex-col items-center justify-center max-w-7xl mx-auto w-full min-w-0 gap-4 py-6">
        ...
        {/* Small auto-playing branch video channel */}
        <div className="w-full max-w-2xl px-1 sm:px-2">
          <div className="rounded-2xl border border-purple-400/30 bg-slate-950/70 backdrop-blur-xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
              <div className="text-xs sm:text-sm font-semibold text-slate-100">Pest Control Video Channel</div>
              <div className="text-[11px] text-purple-300">Video {currentVideo + 1} of {branchVideos.length}</div>
            </div>
            <video
              key={currentVideo}
              src={branchVideos[currentVideo]}
              autoPlay
              muted
              controls
              playsInline
              onEnded={() => setCurrentVideo((currentVideo + 1) % branchVideos.length)}
              className="w-full aspect-video bg-black object-contain"
            />
            <div className="flex items-center justify-center gap-2 py-2">
              {branchVideos.map((_, index) => (
                <button
                  key={index}
                  type="button"
                  aria-label={`Play video ${index + 1}`}
                  onClick={() => setCurrentVideo(index)}
                  className={`h-2.5 rounded-full transition-all ${index === currentVideo ? 'w-7 bg-pink-400' : 'w-2.5 bg-slate-500 hover:bg-slate-300'}`}
                />
              ))}
            </div>
          </div>
