...
const TYPE_LABELS: Record<string, string> = {
  ...
  freezer_check: "Freezer / Chiller Checks",
  cleaning_checks: "Cleaning Checks",
  haccp_daily: "HACCP Daily Checks",
  haccp_manager_review: "HACCP Manager Reviews",
};
...
export default function MyDocumentsPage() {
          ...
          {selected.data.map((item: any, idx: number) => (
            <div key={idx} className="rounded-lg bg-slate-800 p-3">
              <div className="flex items-center justify-between gap-3">
                <span className="text-sm font-semibold">{item.task || item.areaEquipment || item.taskKey}</span>
                {item.completed ? (
              ...
              {item.value && <p className="mt-1 text-sm text-slate-400">{item.value}</p>}
              {item.cleaningDate && (
  ...
  const grouped = docs.reduce<Record<string, MyDocument[]>>((acc, doc) => {
    const type = doc.document_type.startsWith("freezer_check") ? "freezer_check" : doc.document_type;
    if (!acc[type]) acc[type] = [];
    acc[type].push(doc);
    return acc;
  }, {});

  const orderedTypes = TYPE_ORDER.filter(type => grouped[type]?.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4">
      ...
      <div className="mb-4 rounded-xl bg-slate-800 p-4">
        <p className="text-sm text-slate-400">Total Documents: <span className="font-bold text-white">{total}</span></p>
        {orderedTypes.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-2">
            {orderedTypes.map(type => (
              <span key={type} className="rounded-full bg-slate-700 px-3 py-1 text-xs text-slate-300">
      ...
      {loading ? (
        ...
        <p className="text-slate-400">No operational records saved yet.</p>
      ) : (
        <div className="space-y-6">
          {orderedTypes.map(type => (
            <div key={type}>
              <h2 className="mb-2 text-lg font-bold text-slate-200">{TYPE_LABELS[type] || type}</h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="flex items-center gap-1 text-sm">
                        {doc.status === "Completed" || (doc.data.every((i: any) => i.completed)) ? (
                          <CheckCircle2 className="h-4 w-4 text-emerald-400" />
                        ) : (
                          <XCircle className="h-4 w-4 text-red-400" />
                        )}
                        <span className={doc.status === "Completed" || (doc.data.every((i: any) => i.completed)) ? "text-emerald-400" : "text-red-400"}>
                          {doc.status || (doc.data.every((i: any) => i.completed) ? "Completed" : "Incomplete")}
                        </span>
                      </span>
                    </div>
                  </button>
