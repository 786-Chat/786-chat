...
export function HaccpView() {
  ...
  async function fetchFlowConfirmation() {
    try {
      const res = await fetch("/api/haccp-flow-confirmations");
      if (!res.ok) throw new Error("Failed to load flow confirmation");
      const data = await res.json();
      if (data) {
        setFlowConfirmation(data);
        setFlowForm({
