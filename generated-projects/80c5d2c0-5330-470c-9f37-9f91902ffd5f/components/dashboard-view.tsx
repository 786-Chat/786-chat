...
export function DashboardView() {
  ...
  async function fetchAll() {
    try {
      ...
      if (!prodRes.ok || !prodRecRes.ok || !ingRes.ok || !freezersRes.ok || !tempRes.ok || !cleanRes.ok || !haccpRes.ok || !docsRes.ok || !invRes.ok || !stockAdjRes.ok || !invAdjRes.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard icon={FileText} label="Documents" value={documents.length.toString()} />
      </div>

      <Card>
        <CardContent className="p-4">
          <h2 className="mb-3 flex items-center gap-2 text-lg font-bold"><AlertTriangle className="h-5 w-5 text-amber-400" /> Alerts</h2>
          {alerts.length === 0 ? (
            ...
            <ul className="space-y-2">
              {alerts.slice(0, 10).map((alert, i) => (
                <li key={i} className="flex items-start gap-2 text-sm">
                  <Badge tone={alert.tone}>{alert.tone === "red" ? "Critical" : "Warning"}</Badge>
      ...
      <Card>
        <CardContent className="p-4">
          <h2 className="mb-3 flex items-center gap-2 text-lg font-bold"><Activity className="h-5 w-5 text-sky-400" /> Recent Activity</h2>
          {recent.length === 0 ? (
            ...
            <ul className="space-y-2">
              {recent.map((item, i) => (
                <li key={i} className="flex items-start gap-2 text-sm">
                  <span className="shrink-0 text-xs text-slate-500">{new Date(item.time).toLocaleString()}</span>
