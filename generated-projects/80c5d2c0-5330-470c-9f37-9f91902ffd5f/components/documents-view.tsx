...
import { Search, Upload, X, ZoomIn, ZoomOut, Maximize, Minus, Printer, Download, FileText, Image as ImageIcon } from "lucide-react";
                      ...
                      <tr key={doc.id} className="text-slate-300">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <button onClick={() => { if (doc.file_type === "application/pdf") { window.open(`/api/uploads/${doc.id}/content`, "_blank", "noopener,noreferrer"); } else { setViewing(doc); } }} className="cursor-pointer rounded bg-sky-500 px-3 py-1.5 text-xs font-semibold text-slate-950 hover:bg-sky-400">View</button>
      ...
      <div className="flex-1 overflow-auto p-4">
        {isPdf ? (
          <div className="mx-auto flex h-full max-w-full flex-col items-center">
            <p className="mb-2 text-xs text-slate-400">Scroll inside the PDF to view every page. The browser PDF controls can also jump to a page, zoom, print and download.</p>
            <iframe
              ref={iframeRef}
              src={`${contentUrl}#toolbar=1&navpanes=0&zoom=${Math.round(zoom * 100)}`}
          ...
          <div className="flex h-full items-center justify-center">
            <div className="rounded border border-slate-700 bg-white p-4 shadow-lg" style={{ maxWidth: "100%", maxHeight: "100%" }}>
              <img src={contentUrl} alt={doc.title} className="max-h-[80vh] max-w-full object-contain" />
            </div>
          </div>
        )}
      </div>
