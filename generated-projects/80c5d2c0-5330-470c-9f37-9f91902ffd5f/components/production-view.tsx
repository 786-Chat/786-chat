...
export function ProductionView() {
  ...
  const handleSaveRecord = async () => {
    ...
    for (let i = 0; i < ingredientUsage.length; i++) {
      const usage = ingredientUsage[i];
      if (!usage.ingredientId || !usage.supplierBatch || !usage.quantityUsed || !usage.unit) {
        usageErrors.push(`Ingredient ${usage.ingredientName} usage incomplete`);
      } else {
        const stock = ingredients.find((ing) => ing.id === usage.ingredientId);
        if (stock) {
          const available = parseFloat(stock.quantity);
          const used = parseFloat(usage.quantityUsed);
          if (isNaN(used) || used > available) {
            usageErrors.push(`Insufficient stock for ${usage.ingredientName}. Available: ${stock.quantity} ${stock.unit}`);
          }
        }
      }
    }
    if (usageErrors.length > 0) {
      setValidationErrors([...missing, ...usageErrors]);
      return;
    }
    ...
    try {
      const response = await fetch("/api/production-records", {
        ...
        body: JSON.stringify({ ...record, ingredientUsage })
      });
      if (!response.ok) {
