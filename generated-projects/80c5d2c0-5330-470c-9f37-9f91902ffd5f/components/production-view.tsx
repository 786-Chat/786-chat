...
export function ProductionView() {
  ...
  const handleSaveRecord = () => {
    ...
    // New validation: if storage instruction is "Keep Frozen", freezer temperature must be <= -12°C
                ...
                <ul className="mt-1 list-inside list-disc">
                  {validationErrors.map((key) => {
                    if (key === "freezerTemperatureInvalid") {
                      return (
                        <li key={key}>
                          Freezer temperature must be -12°C or colder when storage instruction is "Keep Frozen" (UK guidance recommends -18°C).
                        </li>
                      );
                    }
