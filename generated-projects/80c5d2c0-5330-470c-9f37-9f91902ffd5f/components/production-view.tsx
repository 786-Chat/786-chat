...
const KULFI_INGREDIENTS = "Milk, Cream, Sugar, Condensed Milk, Cardamom, Nuts";
...
function getStorageTemperatureError(instruction: string, value: string): string | null {
  // No instruction selected or no value entered: no validation
  ...
  if (instruction === "Keep Frozen") {
    if (num >= 0) return "Storage Temperature must be negative for frozen storage (target -18°C or colder).";
  ...
  // Store in a Cool Dry Place: no validation
  return null;
}
...
export function ProductionView() {
  ...
  useEffect(() => {
    if (savedRecord && qrRef.current) {
      const url = `${window.location.origin}/production?scan=${encodeURIComponent(savedRecord.batchRecordId || "")}`;
        ...
        <Card className="border-2 border-slate-600 bg-white text-slate-900">
          <CardContent className="p-4 sm:p-6">
            ...
            <dl className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
              ...
              <div className="flex flex-col">
                <dt className="text-sm font-semibold text-slate-500">Ingredients</dt>
                <dd className="text-base font-medium">{scanView.ingredients || "—"}</dd>
              </div>
              <div className="flex flex-col">
        ...
        <Card className="border-2 border-slate-600 bg-white text-slate-900">
          <CardContent className="p-4 sm:p-6">
            ...
            <dl className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
              ...
              {savedRecord.correctiveAction && (
                <div className="flex flex-col">
                  <dt className="text-sm font-semibold text-slate-500">Corrective Action</dt>
                  <dd className="text-base font-medium text-slate-900">{savedRecord.correctiveAction}</dd>
                </div>
              )}
            </dl>
            ...
            {/* Hidden canvases for label generation */}
  ...
  const showCorrectiveAction = storageTempError !== null;

  const coolingFinalTemp = parseFloat(formData.coolingFinalTemperature || "");
  const readyForFreezer = !isNaN(coolingFinalTemp) && coolingFinalTemp < 8;
        ...
        <CardContent className="p-4 sm:p-6">
          ...
          <div className="mb-6 rounded border border-slate-300 bg-slate-50 p-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <span className={`rounded px-2 py-1 font-medium ${readyForFreezer ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                {readyForFreezer ? "Below 8°C ✓" : "Below 8°C"}
              </span>
              <span className="text-slate-400">→</span>
            ...
            {!readyForFreezer && formData.coolingFinalTemperature && (
          ...
          <form className="space-y-4">
            ...
            {showCorrectiveAction && (
              <div className="flex flex-col">
                <label className="mb-1 text-sm font-semibold">Corrective Action</label>
                <textarea
                  className="w-full cursor-text rounded border-2 border-slate-400 bg-white px-3 py-2 text-base focus:border-sky-500 focus:outline-none"
                  rows={3}
              ...
              <div className="rounded border border-red-300 bg-red-50 p-3 text-sm text-red-700">
                ...
                <ul className="mt-1 list-inside list-disc">
                  {validationErrors.map((key) => {
                    ...
                    return <li key={key}>{field ? field.label : key === "labelOk" ? "Label OK?" : key === "storageTemperature" ? "Storage Temperature" : key}</li>;
                  })}
                </ul>
              </div>
            )}
