...
export function ProductionView() {
  ...
  const handleSaveRecord = () => {
    ...
    if (coolingDurationExceeded && !formData.correctiveAction) {
      missing.push("correctiveAction");
    }
    if (finalTempTooHigh && !formData.correctiveAction) {
      missing.push("correctiveAction");
    }
      ...
      <Card className="border-2 border-slate-600 bg-white text-slate-900">
        <CardContent className="p-4 sm:p-6">
          ...
          <form className="space-y-4">
                  ...
                  return (
                    <div key={field.key} className="flex flex-col">
                      ...
                      {coolingDurationExceeded && (
                        <p className="mt-1 text-sm font-semibold text-red-600">Cooling duration exceeds 90 minutes. Corrective action required.</p>
                      )}
                      {finalTempTooHigh && (
                        <p className="mt-1 text-sm font-semibold text-red-600">Cooling Final Temperature is 8°C or above. Corrective action required.</p>
                      )}
                      {coolingComplete && (
            ...
            {(showCorrectiveAction || coolingDurationExceeded || finalTempTooHigh) && (
              <div className="flex flex-col">
              ...
              <div className="rounded border border-red-300 bg-red-50 p-3 text-sm text-red-700">
                ...
                <ul className="mt-1 list-inside list-disc">
                  {validationErrors.map((key) => {
                    ...
                    return <li key={key}>{field ? field.label : key === "labelOk" ? "Label OK?" : key === "storageTemperature" ? "Storage Temperature" : key === "correctiveAction" ? "Corrective Action" : key}</li>;
                  })}
                </ul>
              </div>
            )}
