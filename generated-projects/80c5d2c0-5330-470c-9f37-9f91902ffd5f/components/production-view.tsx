...
export function ProductionView() {
  ...
  const handleSaveRecord = () => {
    ...
    if (storageInstruction === "Keep Frozen" && (isNaN(freezerTemp) || freezerTemp >= 0)) {
                ...
                <ul className="mt-1 list-inside list-disc">
                  {validationErrors.map((key) => {
                    if (key === "freezerTemperatureInvalid") {
                      return (
                        <li key={key}>
                          Enter a frozen storage temperature, for example -18°C.
                        </li>
                      );
                    }
                    const field = fields.find((f) => f.key === key);
                    return <li key={key}>{field ? field.label : key === "labelOk" ? "Label OK?" : key === "freezerTemperature" ? "Freezer Temperature" : key}</li>;
                  })}
                </ul>
              </div>
            )}
