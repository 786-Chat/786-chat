      ...
      <Card className="border-2 border-slate-600 bg-white text-slate-900">
        <CardContent className="p-4 sm:p-6">
          ...
          <div className="mb-6 rounded border border-slate-300 bg-slate-50 p-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <span className={`rounded px-2 py-1 font-medium ${coolingComplete ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                {coolingComplete ? "Below 9°C ✓" : "Below 9°C"}
              </span>
              <span className="text-slate-400">→</span>
          ...
          <form className="space-y-4">
            ...
            {/* Ingredient Traceability collapsible section */}
            <div className="rounded border border-slate-300 bg-slate-50">
              ...
              >
                <span className="text-lg font-bold">Ingredient Traceability</span>
                <span className="text-sm text-slate-500">
                  {traceabilityOpen ? "Hide" : "Record supplier batches and quantities used"}
                </span>
              </button>
              {traceabilityOpen && (
                <div className="border-t border-slate-300 p-4">
                  {productIngredientNames.length === 0 ? (
                    <p className="text-sm text-slate-500">Select a product to see its ingredients.</p>
                  ) : (
                      ...
                      {ingredientUsage.map((usage, index) => {
                        ...
                        return (
                          <div key={index} className="grid grid-cols-1 gap-2 sm:grid-cols-5">
                            <div className="flex items-center text-sm font-semibold">{usage.ingredientName}</div>
                            <select
                              className="h-11 rounded border-2 border-slate-400 bg-white px-3 text-base focus:border-sky-500 focus:outline-none sm:col-span-2"
                              value={usage.ingredientId}
                              onChange={(e) => {
                                const ing = ingredients.find((i) => i.id === e.target.value);
                                handleIngredientUsageChange(index, "ingredientId", e.target.value);
                                handleIngredientUsageChange(index, "supplierBatch", ing ? ing.supplier_batch : "");
                                handleIngredientUsageChange(index, "unit", ing ? ing.unit : "");
                              }}
                            >
                              <option value="">Select stock record</option>
                              {ingredients
                                .filter((ing) => ing.name === usage.ingredientName)
                                .map((ing) => (
                                  <option key={ing.id} value={ing.id}>
                                    {ing.supplier} {ing.supplier_batch} (Available: {ing.quantity} {ing.unit})
                                  </option>
                                ))}
                            </select>
                            <input
                              ...
                              className="h-11 rounded border-2 border-slate-400 bg-white px-3 text-base focus:border-sky-500 focus:outline-none"
                              value={usage.quantityUsed}
                              onChange={(e) => handleIngredientUsageChange(index, "quantityUsed", e.target.value)}
                            />
                            <input
                              type="text"
                              placeholder="Unit"
                              className="h-11 rounded border-2 border-slate-400 bg-white px-3 text-base focus:border-sky-500 focus:outline-none"
                              value={usage.unit}
                              onChange={(e) => handleIngredientUsageChange(index, "unit", e.target.value)}
                              readOnly
                            />
                            {stock && (
