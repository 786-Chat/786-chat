...
import { CheckCircle2, Snowflake, Thermometer, PackageCheck, ArrowRight, Flame, Gauge, Clock, RefreshCw } from "lucide-react";
...
const fields = [
  ...
  { key: "product", label: "Product", type: "select", options: [] as string[] },
...
const requiredFields = [
  ...
  "storageLocation", "storageInDate", "useByDate", "storageInstruction",
...
export function ProductionView() {
  ...
  const [productIngredientNames, setProductIngredientNames] = useState<string[]>([]);
  ...
  const handleChange = (key: string, value: string) => {
    setFormData((prev) => {
      ...
      if (key === "date" && value && next.product) {
        const product = products.find((p) => p.name === next.product);
        if (product) {
          next.useByDate = addDays(value, product.shelf_life_days);
        }
      }
      return next;
    });
  ...
  const handleProductChange = (value: string) => {
    ...
    if (formData.date) {
      updates.useByDate = addDays(formData.date, product.shelf_life_days);
    }
    setFormData((prev) => ({ ...prev, ...updates }));

    // Parse product ingredient names (comma-separated)
    const names = product.ingredients.split(",").map((s) => s.trim()).filter(Boolean);
    setProductIngredientNames(names);
    // Initialize usage rows with empty selections
    setIngredientUsage(names.map((name) => ({
      ingredientId: "",
      ingredientName: name,
      supplierBatch: "",
      quantityUsed: "",
  ...
  const handleIngredientUsageChange = (index: number, field: keyof IngredientUsage, value: string) => {
  ...
  const handleSaveRecord = async () => {
    ...
    const finalTempTooHigh = !isNaN(finalTemp) && finalTemp >= 9;
    ...
    try {
      ...
      // Refresh ingredients to update available stock
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                className="h-11 w-full cursor-pointer rounded bg-sky-500 px-6 text-base font-semibold text-slate-950 hover:bg-sky-400 sm:w-auto"
              >
                Back to Record
              </button>
            </div>
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded border border-slate-300 bg-slate-50 p-4 text-center">
                ...
                <canvas ref={qrRef} className="mx-auto max-w-full" />
                <button
                  type="button"
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-end">
              <button
                ...
                className="h-11 w-full cursor-pointer rounded bg-slate-800 px-6 text-base font-semibold text-white hover:bg-slate-700 sm:w-auto"
              >
                Print Label
              </button>
              <button
              ...
              <button
                ...
                onClick={() => {
                  const today = new Date().toISOString().slice(0, 10);
                  setFormData({ date: today, storageInDate: today, batchNumber: generateBatchNumber() });
                  setLabelOk("");
                  setValidationErrors([]);
                  setSavedRecord(null);
                }}
                className="h-11 w-full cursor-pointer rounded bg-emerald-600 px-6 text-base font-semibold text-white hover:bg-emerald-500 sm:w-auto"
              >
                New Production
              </button>
            </div>
  ...
  const finalTemp = parseFloat(formData.coolingFinalTemperature || "");
  const finalTempTooHigh = !isNaN(finalTemp) && finalTemp >= 9;
  const coolingComplete = !isNaN(finalTemp) && finalTemp < 9;
  ...
  const startTemp = parseFloat(formData.coolingStartTemperature || "");
  const heatTemp = parseFloat(formData.heatTreatmentTemperature || "");
  const storageTemp = parseFloat(formData.storageTemperature || "");
  const heatOk = !isNaN(heatTemp) && heatTemp >= 85;
  const storageOk = !isNaN(storageTemp) && storageTemp <= -18;

  const steps = [
    { label: "Mixing", icon: RefreshCw, tone: "blue" },
    { label: "Start Temperature", icon: Gauge, tone: "blue", value: formData.coolingStartTemperature ? formatTemperature(formData.coolingStartTemperature) : "" },
    { label: "Heating — 85°C or above", icon: Flame, tone: heatOk ? "green" : "blue", value: formData.heatTreatmentTemperature ? formatTemperature(formData.heatTreatmentTemperature) : "" },
    { label: "Cooling complete", icon: CheckCircle2, tone: coolingComplete ? "green" : "blue" },
    { label: "Below 9°C", icon: Thermometer, tone: coolingComplete ? "amber" : "blue" },
    { label: "Moved to freezer", icon: Snowflake, tone: "freezer" },
    { label: "Freezer storage temperature recorded", icon: PackageCheck, tone: storageOk ? "green" : "freezer", value: formData.storageTemperature ? formatTemperature(formData.storageTemperature) : "" }
  ];
    ...
    <div className="space-y-6">
      ...
      <Card className="border-2 border-slate-600 bg-white text-slate-900">
        <CardContent className="p-4 sm:p-6">
          ...
          <div className="mb-6 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-sky-50 p-4 sm:p-6">
            ...
            {!coolingComplete && formData.coolingFinalTemperature && (
              <p className="mt-2 text-sm font-semibold text-amber-600">
                Cooling Final Temperature must be below 9°C before transferring to freezer.
              </p>
            )}
          </div>
                    ...
                    <div key={field.key} className="flex flex-col">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row">
                        ...
                        <button
                          ...
                          className="h-11 w-full shrink-0 cursor-pointer rounded border-2 border-slate-400 bg-slate-100 px-3 text-sm font-semibold text-slate-700 hover:bg-slate-200 sm:w-auto"
                        >
                          Use current time
                        </button>
                      </div>
                  ...
                  return (
                    <div key={field.key} className="flex flex-col">
                      ...
                      <input
                        ...
                        onChange={(e) => handleChange(field.key, e.target.value)}
                      />
                      <p className="mt-1 text-sm text-slate-500">Target: below 9°C before freezer transfer.</p>
                      ...
                      {finalTempTooHigh && (
                        <p className="mt-1 text-sm font-semibold text-red-600">Cooling Final Temperature must be below 9°C. Correct the temperature.</p>
                      )}
                      {coolingComplete && (
