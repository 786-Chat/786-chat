...
export function ProductionView() {
  ...
  const handleChange = (key: string, value: string) => {
    setFormData((prev) => {
      ...
      if (key === "date" && value && !next.freezerInDate) {
        next.freezerInDate = value;
      }
      return next;
    });
  };
  ...
  const handleSaveRecord = () => {
    ...
    const freezerTemp = parseFloat(formData.freezerTemperature);
    if (storageInstruction === "Keep Frozen" && (isNaN(freezerTemp) || freezerTemp > -12)) {
      missing.push("freezerTemperature");
      setValidationErrors([...missing, "freezerTemperatureInvalid"]);
      return;
    }
      ...
      <html>
        ...
        <body>
          <div class="label">
            ...
            <div class="row"><span class="row-label">Storage Temperature:</span> <span>${savedRecord.freezerTemperature ? formatTemperature(savedRecord.freezerTemperature) : ""}</span></div>
            ...
            <dl className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
              ...
              <div className="flex flex-col">
                ...
                <dd className="text-base font-medium">
                  {scanView.freezerTemperature ? formatTemperature(scanView.freezerTemperature) : "—"}
                </dd>
              </div>
            </dl>
            ...
            <dl className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
              ...
              <div className="flex flex-col">
                ...
                <dd className="text-base font-medium text-slate-900">
                  {savedRecord.freezerTemperature ? formatTemperature(savedRecord.freezerTemperature) : "—"}
                </dd>
              </div>
            </dl>
          ...
          <form className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col">
                ...
                {formData.freezerTemperature && (
                  <p className="mt-1 text-sm text-slate-500">
                    Display: {formatTemperature(formData.freezerTemperature)}
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="rounded border border-red-300 bg-red-50 p-3 text-sm text-red-700">
                ...
                <ul className="mt-1 list-inside list-disc">
                  {validationErrors.map((key) => {
                    if (key === "freezerTemperatureInvalid") {
                      return (
                        <li key={key}>
                          Freezer temperature must be -12°C or colder when storage instruction is "Keep Frozen" (UK guidance recommends -18°C). Positive temperatures like +23°C are not acceptable.
                        </li>
                      );
                    }
