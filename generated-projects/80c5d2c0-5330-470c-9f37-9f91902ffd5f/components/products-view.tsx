                ...
                {products.map((p) => (
                  <div key={p.id} className="flex flex-col rounded-xl border border-slate-700 bg-slate-900 p-3 shadow-sm">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button onClick={() => handleEdit(p)} className="flex-1 cursor-pointer rounded bg-sky-500 px-2 py-1.5 text-xs font-semibold text-slate-950 hover:bg-sky-400">Edit</button>
                      <button onClick={() => handleDelete(p.id)} className="flex-1 cursor-pointer rounded bg-red-500 px-2 py-1.5 text-xs font-semibold text-white hover:bg-red-400">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
