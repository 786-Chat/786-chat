...
export function InventoryView() {
  ...
  const filteredItems = items.filter(item => {
    ...
    if (filterFlavour && item.flavour !== filterFlavour) return false;
    if (filterBatch && item.batch_number !== filterBatch) return false;
    if (filterLocation && item.storage_location !== filterLocation) return false;
    if (filterStatus && getStatus(item).label !== filterStatus) return false;
    if (filterDate && item.use_by_date !== filterDate) return false;
    return true;
  });

  const products = Array.from(new Set(items.map(i => i.product_name)));
  const flavours = Array.from(new Set(items.map(i => i.flavour)));
  const batches = Array.from(new Set(items.map(i => i.batch_number)));
  const locations = Array.from(new Set(items.map(i => i.storage_location)));
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select value={filterProduct} onChange={(e) => setFilterProduct(e.target.value)} className="h-10 rounded border-2 border-slate-700 bg-slate-900 px-3 text-sm text-slate-200 focus:border-sky-500 focus:outline-none">
              <option value="">All Products</option>
              {products.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            <select value={filterFlavour} onChange={(e) => setFilterFlavour(e.target.value)} className="h-10 rounded border-2 border-slate-700 bg-slate-900 px-3 text-sm text-slate-200 focus:border-sky-500 focus:outline-none">
              <option value="">All Flavours</option>
              {flavours.map(f => <option key={f} value={f}>{f}</option>)}
            </select>
            <select value={filterBatch} onChange={(e) => setFilterBatch(e.target.value)} className="h-10 rounded border-2 border-slate-700 bg-slate-900 px-3 text-sm text-slate-200 focus:border-sky-500 focus:outline-none">
