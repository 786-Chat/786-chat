...
export function StockView() {
  ...
  async function handleDelete(item: StockItem) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/ingredients/${encodeURIComponent(item.id)}`, {
        method: "DELETE",
        cache: "no-store"
      });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        const message = typeof payload?.error === "string" ? payload.error : payload?.error?.message || "Failed to delete stock item";
  ...
  const filteredItems = items.filter((item) => {
    ...
    if (q && !item.name.toLowerCase().includes(q) && !item.supplier.toLowerCase().includes(q) && !item.supplier_batch.toLowerCase().includes(q)) return false;
  ...
  return (
    <div className="space-y-6">
      ...
      {error && <div className="rounded border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-300">{error}</div>}
      {items.length > 0 && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search product, supplier or batch" className="h-10 w-full rounded border-2 border-slate-700 bg-slate-900 pl-9 pr-3 text-sm text-slate-200 focus:border-sky-500 focus:outline-none" />
          </div>
      ...
      <Card>
        <CardContent className="p-0">
          {loading ? (
            ...
            <p className="p-4 text-sm text-slate-400">No raw stock received yet</p>
          ) : (
            <>
                        ...
                        <tr key={item.id} className="text-slate-300">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <button type="button" onClick={() => setViewing(item)} className="cursor-pointer whitespace-nowrap rounded bg-sky-500 px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-sky-400">View Stock</button>
                              <button type="button" onClick={() => void handleDelete(item)} disabled={deletingId === item.id} title="Delete stock" aria-label={`Delete ${item.name} from stock`} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-red-600 text-white shadow-sm hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="max-h-[90dvh] w-full max-w-2xl overflow-y-auto overflow-x-hidden rounded-xl border border-slate-700 bg-slate-900 p-4 sm:p-6">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setViewing(null)} className="h-10 rounded border border-slate-600 px-4 text-sm font-semibold text-slate-200 hover:bg-slate-800">Close</button>
              <button type="button" onClick={() => void handleDelete(viewing)} disabled={deletingId === viewing.id} className="inline-flex h-10 items-center justify-center gap-2 rounded bg-red-600 px-4 text-sm font-semibold text-white hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50">
