...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  const db = getDb();
  const rows = await db`SELECT * FROM ingredients WHERE id = ${params.id}`;
  if (rows.length === 0) {
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const db = getDb();
  const existing = await db`SELECT * FROM ingredients WHERE id = ${params.id}`;
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  const db = getDb();
  const result = await db`DELETE FROM ingredients WHERE id = ${params.id} RETURNING id`;
  if (result.length === 0) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
