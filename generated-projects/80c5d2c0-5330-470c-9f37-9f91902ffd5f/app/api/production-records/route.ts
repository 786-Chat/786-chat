...
export async function POST(request: Request) {
  ...
  const productRows = await db`SELECT id FROM products WHERE name = ${data.product} AND flavour = ${data.flavour} LIMIT 1`;
  const productId = productRows.length > 0 ? productRows[0].id : null;
  await db`
    INSERT INTO inventory_items (id, production_record_id, product_id, product_name, flavour, batch_number, production_date, quantity_produced, quantity_available, unit, net_weight, storage_location, storage_temperature, storage_instruction, use_by_date)
    VALUES (${inventoryId}, ${data.batchRecordId}, ${productId}, ${data.product}, ${data.flavour}, ${data.batchNumber}, ${data.date}, ${data.quantityMade}, ${data.quantityMade}, ${data.unit}, ${data.netWeight}, ${data.storageLocation}, ${data.storageTemperature ?? null}, ${data.storageInstruction}, ${data.useByDate})
  `;
