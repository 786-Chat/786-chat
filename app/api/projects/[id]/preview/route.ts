...
function normalizeFiles(files: unknown): Record<string, string> {
  ...
  return Object.fromEntries(Object.entries(files as Record<string, unknown>).filter((entry): entry is [string, string] => typeof entry[0] === "string" && typeof entry[1] === "string"))
}
...
function routeFileCandidates(route: string) {
  ...
  return [`app/${segment}/page.tsx`, `app/${segment}/page.jsx`, `pages/${segment}.tsx`, `pages/${segment}.jsx`, `pages/${segment}/index.tsx`, `pages/${segment}/index.jsx`]
}
...
function runtimeWorkerHtml(projectId: string, projectName: string, route: string) {
  const base = String(process.env.RUNTIME_WORKER_URL || process.env.NEXT_PUBLIC_RUNTIME_WORKER_URL || "").trim().replace(/\/+$/, "")
  if (!base) return ""
...
function staticHtml(files: Record<string, string>, projectName: string, route: string) {
  ...
  if (!pageCode) return errorHtml(`The project does not contain a page for ${route}. Expected ${routeFileCandidates(route)[0]}.`)
  const jsx = extractReturnJsx(pageCode)
  const body = jsxToHtml(jsx)
  if (!body.trim() || body.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().length < 4) return errorHtml(`The ${route} page did not produce visible preview content.`)
...
export async function GET(request: Request, { params }: { params: { id: string } | Promise<{ id: string }> }) {
  try {
    ...
    const rows = await sql`SELECT id, name, files FROM projects WHERE id = ${projectId}::uuid AND user_id = ${session.id}::uuid AND deleted_at IS NULL LIMIT 1`
    if (!rows.length) {
      const html = errorHtml("Project not found")
      return rawHtml ? new Response(html, { status: 404, headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" } }) : NextResponse.json({ error: "Project not found" }, { status: 404 })
    }
    ...
    if (rawHtml) return new Response(html, { status: 200, headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store, no-cache, must-revalidate" } })
    return NextResponse.json({ success: true, html, route }, { headers: { "Cache-Control": "no-store" } })
  } catch (error) {
    ...
    return NextResponse.json({ error: "Failed to build project preview", debug: message }, { status: 500 })
  }
}
