...
async function isAdmin(): Promise<boolean> {
  ...
  if (email === "mujeeb@job4u.com" || email === "admin@mujeebproai.com") {
    return true
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedFeatures = Array.isArray(features) ? [...features] : []

    if (updatedFeatures.length > 0) {
      updatedFeatures[0] = `${Number(messages_included || 0).toLocaleString()} AI messages/month`
    }

    const priceNumber = Number(price || 0)
    const extraPriceNumber = Number(extra_message_price || 0)
    const messagesNumber = Number(messages_included || 0)

    const priceUsd = Math.round(priceNumber * 1.3 * 100) / 100
    const priceEur = Math.round(priceNumber * 1.17 * 100) / 100
    const pricePkr = Math.round(priceNumber * 350)

    const extraUsd = Math.round(extraPriceNumber * 1.3 * 1000) / 1000
    const extraEur = Math.round(extraPriceNumber * 1.17 * 1000) / 1000
    const extraPkr = Math.round(extraPriceNumber * 350 * 10) / 10
    ...
    await sql`
      ...
      VALUES (
        ${id}, ${name},
        ${priceNumber}, ${priceUsd}, ${priceEur}, ${pricePkr},
        ${messagesNumber},
        ${extraPriceNumber}, ${extraUsd}, ${extraEur}, ${extraPkr},
        ${JSON.stringify(updatedFeatures)}::jsonb, ${Boolean(is_popular)}, ${Boolean(is_enabled)}, ${Number(sort_order || 0)},
        ${priceNumber > 0}, NOW()
      )
